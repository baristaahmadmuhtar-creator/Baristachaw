...
export const PRICING: Record<'plus' | 'pro', PlanPricing> = {
  ...
  pro: {
    ...
    quarterly: {
      ...
      discountPct: 20,
      saveLabel: { id: 'Hemat 20%', en: 'Save 20%' },
    },
    yearly: {
      original: { idr: 1_800_000, bnd: 54.00, usd: 45.00 },
      discounted: { idr: 999_000, bnd: 30.00, usd: 25.00 },
      discountPct: 44,
      saveLabel: { id: 'Hemat 44%', en: 'Save 44%' },
    },
  },
};

export function formatCurrency(amount: number, currency: 'idr' | 'bnd' | 'usd'): string {
  switch (currency) {
