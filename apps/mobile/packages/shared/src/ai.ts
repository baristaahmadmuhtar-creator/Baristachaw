...
export const BARISTA_SKILL_FOCUS_VALUES: readonly AgentBaristaSkillFocus[] = [
  'espresso_dial_in',
  'brew_recipe_design',
  'sensory_cupping',
  'milk_latte_art',
...
export const AI_CHAT_INTENT_VALUES: readonly AiChatIntent[] = [
  'greeting',
  'question',
  'command',
  'recipe_request',
...
export const AI_TOOL_SUGGESTION_VALUES: readonly AiToolSuggestion[] = [
  'ai_brew',
  'brew_timer',
  'ratio_calculator',
  'vision_scan',
...
export function resolveAgentProfileNamespace(userId?: string | null): string {
  const normalizedUserId = normalizeProfileText(userId, 120);
  return normalizedUserId ? `user:${normalizedUserId}` : 'guest';
}

function mergeAgentProfileMemory(
  previousProfile?: Partial<AgentProfileMemory> | null,
  updates?: Partial<AgentProfileMemory> | null,
): AgentProfileMemory {
...
function extractUserDisplayName(text: string): string | undefined {
  const match = text.match(/(?:\bpanggil saya\b|\bsebut saya\b|\bcall me\b)\s+([a-z0-9 ._'\u2019-]{2,40})/i);
  return normalizeProfileText(match?.[1], 60);
}

function extractAssistantName(text: string): string | undefined {
  const match = text.match(/(?:\bnama kamu\b|\bpanggil dirimu\b|\bcall yourself\b|\byour name is\b)\s+([a-z0-9 ._'\u2019-]{2,40})/i);
  return normalizeProfileText(match?.[1], 60);
}

function extractDetailPreference(text: string): ResponseVerbosity | undefined {
  if (/\b(?:jawab|reply|respond|be)\s+(?:lebih\s+)?(?:ringkas|singkat|brief|short)\b/i.test(text)) return 'short';
  if (/\b(?:jawab|reply|respond|be)\s+(?:lebih\s+)?(?:detail|rinci|lengkap|panjang|detailed|comprehensive)\b/i.test(text)) return 'comprehensive';
  if (/\b(?:jawab|reply|respond)\s+(?:seimbang|balanced)\b/i.test(text)) return 'balanced';
  return undefined;
}

function extractTonePreference(text: string): ResponseTone | undefined {
  if (/\b(?:pakai|gunakan|use|reply in|jawab dengan)\s+(?:tone\s+)?(?:formal|professional|profesional)\b/i.test(text)) return 'professional';
  if (/\b(?:pakai|gunakan|use|reply in|jawab dengan)\s+(?:tone\s+)?(?:ramah|friendly|casual|santai)\b/i.test(text)) return 'friendly';
  if (/\b(?:pakai|gunakan|use|reply in|jawab dengan)\s+(?:tone\s+)?(?:netral|neutral)\b/i.test(text)) return 'neutral';
  return undefined;
}

function extractResponseStyle(text: string): ResponseFormat | undefined {
  if (/\b(?:pakai|gunakan|jawab|reply)\s+(?:format\s+)?(?:bullet|bullets|poin|list)\b/i.test(text)) return 'bullets';
  if (/\b(?:pakai|gunakan|jawab|reply)\s+(?:format\s+)?(?:langkah|steps|step-by-step)\b/i.test(text)) return 'steps';
  if (/\b(?:pakai|gunakan|jawab|reply)\s+(?:format\s+)?(?:table|tabel)\b/i.test(text)) return 'table';
  if (/\b(?:pakai|gunakan|jawab|reply)\s+(?:format\s+)?(?:plain|paragraph|paragraf)\b/i.test(text)) return 'plain';
  return undefined;
}

function extractEmojiPolicy(text: string): AgentProfileMemory['emojiPolicy'] | undefined {
  if (/\b(?:jangan pakai emoji|tanpa emoji|no emoji|without emoji)\b/i.test(text)) return 'none';
  if (/\b(?:sedikit emoji|minimal emoji|few emoji)\b/i.test(text)) return 'minimal';
  if (/\b(?:pakai emoji|gunakan emoji|use emoji)\b/i.test(text)) return 'default';
  return undefined;
}
...
export function inferBaristaSkillFocus(
  inputText: string,
...
export function formatBaristaSkillFocusForPrompt(skills: readonly AgentBaristaSkillFocus[]): string {
...
export function extractDurablePreferenceUpdates(
  inputText: string,
  previousProfile?: Partial<AgentProfileMemory> | null,
): Partial<AgentProfileMemory> {
  const text = String(inputText || '').trim();
  if (!text) return {};

  const durable = containsDurableCue(text) || text.length <= 72;
  const updates: Partial<AgentProfileMemory> = {};

  const preferredLanguage = detectExplicitLanguageSwitch(text);
  if (preferredLanguage) updates.preferredLanguage = preferredLanguage;

  const userDisplayName = extractUserDisplayName(text);
  if (userDisplayName) updates.userDisplayName = userDisplayName;

  const assistantName = extractAssistantName(text);
  if (assistantName) updates.assistantName = assistantName;

  const detailPreference = extractDetailPreference(text);
  if (durable && detailPreference) updates.detailPreference = detailPreference;

  const tonePreference = extractTonePreference(text);
  if (durable && tonePreference) updates.tonePreference = tonePreference;

  const responseStyle = extractResponseStyle(text);
  if (durable && responseStyle) updates.responseStyle = responseStyle;

  const emojiPolicy = extractEmojiPolicy(text);
  if (durable && emojiPolicy) updates.emojiPolicy = emojiPolicy;

  const skillFocus = sortSkillFocusScores(scoreBaristaSkillFocusText(text));
  if (skillFocus.length && (containsDurableCue(text) || hasDurableSkillFocusCue(text))) {
    updates.skillFocus = skillFocus;
  }
  ...
  return mergeAgentProfileMemory(previousProfile, updates);
}

export function formatAgentProfileForPrompt(profile?: Partial<AgentProfileMemory> | null): string {
...
function looksLikeUtf8Mojibake(text: string) {
  // Common mojibake markers when UTF-8 is decoded as latin-1/cp1252.
...
export function resolveUserLanguage(
  ...
  const fromApp = normalizeLanguageTag(appLanguage);
  if (fromApp) return fromApp;
  const fromHeader = normalizeLanguageTag(acceptLanguage);
...
function hasStrongStandaloneIntentSignal(text: string): boolean {
  return (
    isRecipeIntent(text)
    || isDiagnosticIntent(text)
    || isSaveCommandIntent(text)
    || isToolCommandIntent(text)
    || isAppSupportIntent(text)
    || containsDurableCue(text)
    || hasDurableSkillFocusCue(text)
  );
}

function isAmbiguousContinuationRequest(text: string): boolean {
  const trimmed = String(text || '').trim();
  if (!trimmed || !hasContinuationCue(trimmed)) return false;
  if (hasStrongStandaloneIntentSignal(trimmed)) return false;
  const words = trimmed.split(/\s+/).filter(Boolean);
  if (words.length <= 10) return true;
  return hasAny(trimmed, [
    /\b(which one|yang mana|mana yang|refer yang mana|maksud yang mana)\b/i,
    /\b(itu yang mana|tadi yang mana|which did you mean)\b/i,
  ]);
}
...
function shouldUseConversationCarryover(text: string): boolean {
  ...
  const hasDirectSignal = (
    hasStrongStandaloneIntentSignal(trimmed)
...
export function inferAiToolSuggestions(
  ...
  if (intent === 'diagnostic' && (skills.includes('espresso_dial_in') || skills.includes('troubleshooting'))) {
    scoreToolSuggestion(scores, 'ratio_calculator', 4);
  }
...
export function formatAiChatFlowForPrompt(
  intent: AiChatIntent,
  ...
  if (options?.mode === 'fast') {
    lines.push('- Fast-mode contract: lead with the direct answer. Default to 1-2 short paragraphs or up to 4 short bullets.');
    lines.push('- Fast-mode contract: skip long intros, deep reasoning, and document-style structure unless the user explicitly asks for detail.');
    lines.push('- Fast-mode contract: give at most one next action or one tool suggestion when it materially helps.');
  } else if (options?.mode === 'deep') {
    lines.push('- Deep-mode contract: include reasoning, tradeoffs, and a concrete action plan, not just a short answer.');
    lines.push('- Deep-mode contract: make assumptions explicit and show how the user should validate the result.');
    lines.push('- Deep-mode contract: structure the answer for decisions, not for decoration.');
  } else {
    lines.push('- Normal-mode contract: answer first, then keep only the most useful rationale and next step.');
    lines.push('- Normal-mode contract: use 2-4 short blocks or compact bullets. Keep it easy to scan on mobile.');
    lines.push('- Normal-mode contract: avoid essay-style output unless the user explicitly asks for depth.');
  }
...
function deriveDomainDepth(text: string, mode: ResponseMode): 'basic' | 'advanced' {
...
function deriveAmbiguityPolicy(text: string): AmbiguityPolicy {
  if (isGreetingOnlyInput(text)) return 'ask_first';
  if (isAmbiguousContinuationRequest(text)) return 'ask_first';
...
function deriveAmbiguityRisk(inputText: string): 'low' | 'high' {
  ...
  if (isAmbiguousContinuationRequest(text)) return 'high';
...
export function deriveExpectationProfile(inputText: string, mode: ResponseMode): ExpectationProfile {
  ...
  if (mode === 'fast' && !hasExplicitDetailedRequest(text)) {
    if (intent === 'question' || intent === 'command' || intent === 'open_ended' || intent === 'app_support') {
      if (isShortDirectRequest(text)) {
        verbosity = 'short';
        format = 'plain';
      }
    }
  ...
  if (mode === 'deep' && intent !== 'greeting') {
    if (verbosity !== 'short') verbosity = 'comprehensive';
    if (format === 'plain') format = 'steps';
    domainDepth = 'advanced';
    if (tone === 'neutral') tone = 'professional';
  }
...
export function hasMeaningfulUserMessage(
  messages: Array<Pick<ConversationContextMessage, 'role' | 'text'> | { role?: string; text?: string | null }>,
): boolean {
  return messages.some((message) => {
    const normalized = normalizeConversationMessage(message);
    return normalized?.role === 'user' && normalized.text.length >= 2;
  });
}
...
export function buildConversationSummary(
  ...
  const recentCount = Math.max(1, Math.min(20, options?.recentCount ?? 10));
  const older = normalized.slice(0, Math.max(0, normalized.length - recentCount));
  if (!older.length) return '';

  const sampled = older.slice(Math.max(0, older.length - 8));
  const lines = sampled.map((message) => `${message.role === 'user' ? 'User' : 'Assistant'}: ${message.text}`);

  return [
    options?.sessionTitle ? `Session: ${options.sessionTitle}` : '',
    options?.preferredLanguage ? `Preferred reply language: ${options.preferredLanguage}` : '',
...
export function buildConversationContext(options: {
  ...
  const normalized = options.messages
    .map((message) => normalizeConversationMessage(message))
    .filter((message): message is ConversationContextMessage => Boolean(message));
  const recentCount = Math.max(1, Math.min(20, options.recentCount ?? 10));
  const latestUserText = String(options.latestUserText || '').trim();
  const needsCarryover = latestUserText ? shouldUseConversationCarryover(latestUserText) : true;
  const ambiguousContinuation = latestUserText ? isAmbiguousContinuationRequest(latestUserText) : false;

  let recentMessages: ConversationContextMessage[];
  let computedSummary = '';

  if (!latestUserText) {
    recentMessages = normalized.slice(-recentCount);
    computedSummary = buildConversationSummary(normalized, {
      sessionTitle: options.sessionTitle,
      preferredLanguage: options.preferredLanguage,
      recentCount,
    });
  } else if (!needsCarryover) {
    const latestUserMessage = [...normalized].reverse().find((message) => message.role === 'user');
    recentMessages = latestUserMessage ? [latestUserMessage] : normalized.slice(-1);
  } else if (ambiguousContinuation) {
    const boundedWindow = Math.min(recentCount, options.mode === 'fast' ? 3 : 4);
    recentMessages = normalized.slice(-boundedWindow);
  } else {
    const carryoverWindow = Math.min(recentCount, options.mode === 'fast' ? 6 : options.mode === 'normal' ? 8 : recentCount);
    recentMessages = normalized.slice(-carryoverWindow);
    computedSummary = buildConversationSummary(normalized, {
      sessionTitle: options.sessionTitle,
      preferredLanguage: options.preferredLanguage,
      recentCount: carryoverWindow,
    });
  }

  return {
    summary: computedSummary || (needsCarryover && !ambiguousContinuation ? options.summary : undefined),
    preferredLanguage: normalizeLanguageTag(options.preferredLanguage),
    recentMessages,
    sessionTitle: options.sessionTitle,
  };
}

export function isReusableDraftSession(session: {
  deletedAt?: number;
  ...
  return Number(session.messageCount || 0) <= 1;
}
...
function modePreamble(mode: ResponseMode): string {
  if (mode === 'fast') return 'Mode: FAST. Prioritize speed, directness, and low reading effort.';
  if (mode === 'deep') return 'Mode: DEEP. Prioritize decision quality, explicit reasoning, and validation.';
  return 'Mode: NORMAL. Balance speed, clarity, and usefulness with answer-first structure.';
}
...
function modeOutputContract(mode: ResponseMode): string[] {
  if (mode === 'fast') {
    return [
      'Output contract: return the useful answer immediately.',
      'Output contract: default to 1-2 short paragraphs or up to 4 short bullets.',
      'Output contract: skip backstory, repeated context, and detailed reasoning unless the user explicitly asks for it.',
    ];
  }
  if (mode === 'deep') {
    return [
      'Output contract: start with the conclusion, then show analysis, tradeoffs, and a concrete action plan.',
      'Output contract: surface assumptions, risks, and validation checks when they affect the recommendation.',
      'Output contract: if the request has multiple viable paths, compare them instead of pretending there is only one option.',
    ];
  }
  return [
    'Output contract: answer first, then add only the most useful rationale.',
    'Output contract: use 2-4 short blocks or compact bullets; keep it easy to scan on mobile.',
    'Output contract: include one practical next step when it helps the user act.',
  ];
}
...
export function buildOrchestratedPrompt(
  ...
  const directToolSuggestions = inferAiToolSuggestions(cleaned, chatIntent);
  const chatFlowBlock = formatAiChatFlowForPrompt(chatIntent, directToolSuggestions, {
    mode,
    inputText: cleaned,
    expectation,
  });
  ...
  if (isAmbiguousContinuationRequest(cleaned)) {
    lines.push('');
    lines.push('Continuation guard: the latest user message refers back to earlier context but does not anchor the target clearly.');
    lines.push('Continuation guard: if more than one prior item could match, ask one short clarification instead of choosing for the user.');
    lines.push('Continuation guard: do not continue the older topic just because it is longer, newer, or more detailed.');
  }
...
export function buildChatPromptForMode(userText: string, mode: ResponseMode): string {
  const resolved = buildResponseOrchestration(userText, mode);
  return buildOrchestratedPrompt(userText, mode, resolved);
}

export function appendAttachmentResponseStyle(basePrompt: string, mode: ResponseMode): string {
  if (mode === 'fast') {
    return `${basePrompt}\n\nResponse style: FAST. Lead with the direct answer. Keep it to 1-2 short paragraphs or up to 4 short bullets. Skip detailed reasoning unless explicitly requested.`;
  }
  if (mode === 'deep') {
    return `${basePrompt}\n\nResponse style: DEEP. Be thorough, structured, and decision-oriented. State the conclusion, explain reasoning and tradeoffs, then give a concrete action plan and validation cues.`;
  }
  return `${basePrompt}\n\nResponse style: NORMAL. Answer first, then add only the most useful rationale. Keep it practical, moderately concise, and easy to scan. Avoid long intros and document-style formatting unless the user asked for depth.`;
}
...
export function buildAttachmentSmartPrompt(
  ...
  const subjectLabel = fileName ? `File: ${fileName}` : 'User attachment';
...
${subjectLabel}`;
  ...
  return `You are BaristaClaw, an expert coffee assistant following SCA standards.
Analyze the attached image/video/audio carefully and provide:
1) what is visible/contained
