...
export function localizeAiBrewDynamicText(text: string, language?: string) {
  ...
  const normalized = text
    ...
    .replace(/Iced split source: final beverage ratio 1:(\d+(?:\.\d+)?), cold ratio 1:(\d+(?:\.\d+)?), liquid\/ice (.+)/i, 'Sumber split seduh es: rasio final 1:$1, rasio dingin 1:$2, cair/es $3.')
  ...
  const localized = normalized
    ...
    .replace(/Manual full immersion adapted to (.+?) because the hybrid recovery would sit too close to the (\d+(?:\.\d+)?) ml closed-phase limit on (.+?)\./i, 'Rendaman penuh manual disesuaikan ke $1 karena recovery hybrid terlalu dekat batas fase tertutup $2 ml pada $3.')
    .replace(/Manual full immersion adapted to (.+?) for the (\d+(?:\.\d+)?) ml closed-phase limit on (.+?)\./i, 'Rendaman penuh manual disesuaikan ke $1 mengikuti batas fase tertutup $2 ml pada $3.')
