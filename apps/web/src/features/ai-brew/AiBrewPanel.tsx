...
import { resolveManualPresetChange } from './manualPresetChangeGuard.ts';
import {
...
const COPY = {
  en: {
    ...
    waterDerivationDirect: 'Verified mineral source',
  ...
  id: {
    ...
    waterDerivationDirect: 'Sumber mineral terverifikasi',
...
type AiBrewCoreMetricItem = {
  ...
  value: string;
  detail?: string;
  highlight?: 'water_split';
};

const AI_BREW_STYLE_COPY_KEYS: Partial<Record<AiBrewMethodFamily, Record<string, string>>> = {
  french_press: {
    auto: 'frenchPressStyleAuto',
    traditional: 'frenchPressStyleTraditional',
    clean_decant: 'frenchPressStyleCleanDecant',
    double_filter: 'frenchPressStyleDoubleFilter',
    heavy_concentrate: 'frenchPressStyleHeavyConcentrate',
    sweet_immersion: 'frenchPressStyleSweetImmersion',
  },
  kalita_wave: {
    auto: 'kalitaWaveStyleAuto',
    traditional_flat_three: 'kalitaWaveStyleTraditionalFlatThree',
    competition_fast_four: 'kalitaWaveStyleCompetitionFastFour',
    continuous_slow_stream: 'kalitaWaveStyleContinuousSlowStream',
    iced_wave: 'kalitaWaveStyleIcedWave',
    high_dose_concentrate: 'kalitaWaveStyleHighDoseConcentrate',
  },
  clever_dripper: {
    auto: 'cleverDripperStyleAuto',
    classic_immersion: 'cleverDripperStyleClassicImmersion',
    water_first: 'cleverDripperStyleWaterFirst',
    hybrid_release: 'cleverDripperStyleHybridRelease',
    iced_clever: 'cleverDripperStyleIcedClever',
    high_dose_concentrate: 'cleverDripperStyleHighDoseConcentrate',
  },
  chemex: {
    auto: 'chemexStyleAuto',
    classic_clear: 'chemexStyleClassicClear',
    pulse_control: 'chemexStylePulseControl',
    iced_chemex: 'chemexStyleIcedChemex',
    high_dose_concentrate: 'chemexStyleHighDoseConcentrate',
  },
  moka_pot: {
    auto: 'mokaPotStyleAuto',
    classic: 'mokaPotStyleClassic',
    soft_heat: 'mokaPotStyleSoftHeat',
    milk_base: 'mokaPotStyleMilkBase',
  },
  cold_brew: {
    auto: 'coldBrewStyleAuto',
    immersion: 'coldBrewStyleImmersion',
    concentrate: 'coldBrewStyleConcentrate',
    flash_chill: 'coldBrewStyleFlashChill',
  },
  batch_brew: {
  ...
  april: {
    auto: 'aprilStyleAuto',
    classic: 'aprilStyleClassic',
    two_pour: 'aprilStyleTwoPour',
    high_dose: 'aprilStyleHighDose',
  },
  melitta: {
    auto: 'melittaStyleAuto',
    classic: 'melittaStyleClassic',
    pulse: 'melittaStylePulse',
    iced: 'melittaStyleIced',
  },
  kono: {
    auto: 'konoStyleAuto',
    classic: 'konoStyleClassic',
    slow: 'konoStyleSlow',
    iced: 'konoStyleIced',
  },
};

function humanizeAiBrewStyle(value: string) {
  return value
    .replace(/[_-]+/g, ' ')
...
function insertMetricAfter(
  items: AiBrewCoreMetricItem[],
  metric: AiBrewCoreMetricItem | null,
  afterId: string,
) {
  if (!metric) return items;
  if (items.some((item) => item.id === metric.id)) return items;
  const index = items.findIndex((item) => item.id === afterId);
  if (index < 0) return [...items, metric];
  return [
...
function buildMethodStyleMetricItem(plan: BrewPlan, copy: CopySet, language: string): AiBrewCoreMetricItem | null {
  const id = isIndonesianAiBrewLanguage(language);
  ...
  if (plan.methodFamily === 'hario_switch') {
    return {
      id: 'style',
      label: id ? 'Gaya' : 'Style',
      value: formatPlanSwitchPresetLabel(plan, language),
      detail: id ? 'Preset Switch aktif.' : 'Active Switch preset.',
    };
  }

  const recipeStyle = String(plan.recipeStyle || '').trim();
  if (!recipeStyle || recipeStyle === 'auto') return null;
  const copyKey = AI_BREW_STYLE_COPY_KEYS[plan.methodFamily]?.[recipeStyle];
  const copyRecord = copy as Record<string, string>;
  const value = copyKey ? copyRecord[copyKey] : '';
  return {
    id: 'style',
    label: id ? 'Gaya' : 'Style',
    value: value || humanizeAiBrewStyle(recipeStyle),
    detail: id ? 'Gaya seduh aktif.' : 'Active brew style.',
  };
}
...
function translateWorkflowGuideTextToEnglish(value: string) {
  ...
  const exactMap: Array<[RegExp, string]> = [
    [/Dosis (\d+(?:\.\d+)?\s*g)\.?/i, 'Dose $1.'],
    [/Target waktu (\d+j\s*\d+m|\d+j|\d+m|\d+(?::\d+)?(?:s)?)\.?/i, 'Target time $1.'],
    [/Target (\d+(?:\.\d+)?\s*ml)\.?/i, 'Target $1.'],

    [/No bean-profile modifier active/i, 'Belum ada penyesuaian profil bean yang aktif.'],
    [/Manual mineral input is active for this brew plan/i, 'Data mineral manual digunakan untuk rencana seduh ini.'],
    [/Manual brew preset adapted from the selected dose, water target, or temperature\. The planner kept the preset direction but recalculated ratio, timing, and method guardrails from the current inputs/i, 'Preset seduh disesuaikan dari dosis, target air, atau suhu yang Anda pilih. Arah resep tetap dipertahankan, lalu rasio, waktu, dan batas pengaman metode dihitung ulang dari input saat ini.'],
    [/Manual preset adapted to selected dose and water/i, 'Preset manual disesuaikan dari dosis dan air yang Anda pilih.'],
    [/(\d+(?::\d+)?|\d+\s*s|[\d.]+C) is selected from roast, process, water minerals, and target extraction style/i, '$1 dipilih dari roast, proses, mineral air, dan gaya ekstraksi yang ditargetkan.'],
    [/Finishing after main taste time/i, 'Selesai setelah waktu rasa utama.'],
    [/Direct demineral use is a low-confidence filter experiment; expect a clean cup with light body or hollow risk unless remineralized/i, 'Pemakaian langsung air demineral adalah eksperimen filter berkeyakinan rendah; seduhan bisa clean dengan body ringan atau terasa kosong jika belum diremineralisasi.'],
    [/AI numeric optimizer accepted inside guardrails \(confidence (\d+(?:\.\d+)?)%\)/i, 'Pengoptimal angka AI diterima karena masih berada di dalam batas pengaman (keyakinan $1%).'],
    [/Low-TDS water may need a touch more thermal energy/i, 'Air dengan TDS rendah mungkin butuh sedikit tambahan energi panas.'],
    [/Add water in stages and make sure the coarse bed is fully wet before the long steep starts/i, 'Masukkan air bertahap sampai seluruh bubuk kasar basah merata sebelum rendam panjang dimulai.'],
    [/Leave the vessel covered at the planned temperature; strength comes from time, not stirring/i, 'Tutup wadah sesuai suhu rencana; kekuatan rasa dibangun dari waktu rendam, bukan dari adukan berulang.'],
    [/Separate the concentrate from the grounds first, then dilute or serve over ice after the extraction is stopped/i, 'Pisahkan konsentrat dari ampas terlebih dulu, lalu dilusi atau sajikan di atas es setelah ekstraksi berhenti.'],
    [/Filter cleanly first, then dilute or serve only after the grounds are separated/i, 'Saring sampai bersih terlebih dulu, lalu dilusi atau sajikan hanya setelah ampas terpisah.'],
    [/Filter cleanly/i, 'Saring sampai bersih.'],
    [/This lighter dose needs cleaner contact and shorter idle gaps between checkpoints/i, 'Dosis yang lebih ringan butuh kontak lebih rapi dan jeda antartahap yang tidak terlalu panjang.'],
    [/Higher-TDS water can read fuller and heavier with the same brew settings/i, 'Air dengan TDS lebih tinggi bisa terasa lebih penuh dan berat pada setting seduh yang sama.'],
    [/Water hardness is below the recommended band/i, 'Hardness air berada di bawah rentang rekomendasi.'],
    [/Water hardness is above the recommended band/i, 'Hardness air berada di atas rentang rekomendasi.'],
    [/Water alkalinity is below the recommended band/i, 'Alkalinitas air berada di bawah rentang rekomendasi.'],
    [/Water alkalinity is above the recommended band/i, 'Alkalinitas air berada di atas rentang rekomendasi.'],
    [/Alkaline water can mute acidity; verify manually before treating it as filter friendly/i, 'Air alkaline bisa meredam acidity. Verifikasi manual sebelum dianggap aman untuk seduh filter.'],
    [/Alkaline water can mute acidity\. Verify manually before treating it as filter friendly/i, 'Air alkaline bisa meredam acidity. Verifikasi manual sebelum dianggap aman untuk seduh filter.'],
    [/High alkalinity\/buffer can mute acidity and flatten floral coffees\. Use lower contact time or choose manual minerals for delicate beans/i, 'Alkalinity/buffer tinggi bisa meredam acidity dan membuat kopi floral terasa datar. Pakai contact time lebih rendah atau pilih manual minerals untuk bean yang delicate.'],
    [/High buffer can mute acidity and flatten floral coffees/i, 'Buffer tinggi bisa meredam acidity dan membuat kopi floral terasa datar.'],
    [/Process not specified\. No automatic process modifier was applied/i, 'Proses belum ditentukan. Tidak ada modifier proses otomatis yang dipakai.'],
    [/Variety not specified\. No automatic variety modifier was applied/i, 'Varietas belum ditentukan. Tidak ada modifier varietas otomatis yang dipakai.'],
    [/Wet-hulled Indonesian cue: Dense & Comforting suggested/i, 'Cue wet-hulled Indonesia: Tebal & Nyaman disarankan.'],
    [/Canephora\/non-arabica body cue: Dense & Comforting suggested/i, 'Cue body canephora/non-arabica: Tebal & Nyaman disarankan.'],
    [/High-clarity variety cue: Floral & Transparent suggested/i, 'Cue varietas berkejernihan tinggi: Floral & Transparan disarankan.'],
    [/Washed high-altitude cue: Floral & Transparent suggested/i, 'Cue washed dataran tinggi: Floral & Transparan disarankan.'],
    [/Experimental high-variability process cue: Fruit-Forward suggested/i, 'Cue proses eksperimental variabilitas tinggi: Buah Lebih Menonjol disarankan.'],
    [/Natural or high-aroma process cue: Fruit-Forward suggested/i, 'Cue proses natural atau aroma tinggi: Buah Lebih Menonjol disarankan.'],
    [/Sweet\/body cue: Soft & Round suggested/i, 'Cue manis/body: Lembut & Bulat disarankan.'],
    [/Unknown or weak evidence: Balance & Clean suggested/i, 'Bukti lemah/tidak dikenal: Seimbang & Bersih disarankan.'],
    [/High variability process: use taste feedback before increasing extraction pressure/i, 'Proses variabilitas tinggi: pakai feedback rasa sebelum menaikkan tekanan ekstraksi.'],
    [/Sensory taxonomy cue applied as a conservative baseline, not as a fixed flavor claim/i, 'Cue taxonomy sensory dipakai sebagai baseline konservatif, bukan klaim rasa pasti.'],
    [/Exact device profile unavailable; family fallback was used/i, 'Profil alat presisi belum tersedia; profil keluarga alat digunakan sebagai pengganti.'],
    [/Bean profile left neutral; no bean-specific modifier was applied/i, 'Profil bean dibiarkan netral; tidak ada modifier spesifik bean yang dipakai.'],
    [/Process known: (.+)/i, 'Proses diketahui: $1.'],
    [/Variety known: (.+)/i, 'Varietas diketahui: $1.'],
    [/Roast level known: (.+)/i, 'Level sangrai diketahui: $1.'],
    [/Exact brewer profile matched/i, 'Profil alat exact ditemukan.'],
    [/Exact brewer and workflow validation passed/i, 'Profil alat exact dan validasi panduan sudah lolos.'],
    [/Guardrail or workflow validation blocked this combination/i, 'Guardrail atau validasi panduan memblokir kombinasi ini.'],
    [/Safe baseline used with caution flags/i, 'Acuan aman digunakan bersama catatan kehati-hatian.'],
    [/No process, variety, origin, or bean profile was provided/i, 'Proses, varietas, asal, dan profil bean belum diisi.'],
    [/Some bean detail is missing, so a safe baseline remains active/i, 'Sebagian detail bean belum lengkap, jadi acuan aman tetap digunakan.'],
    [/Process high-variability: validate with taste feedback before increasing extraction/i, 'Proses variabilitas tinggi: validasi dengan feedback rasa sebelum menaikkan ekstraksi.'],
    [/Dark roast: protect bitterness with lower extraction pressure/i, 'Sangrai gelap: lindungi dari pahit dengan tekanan ekstraksi lebih rendah.'],
    [/Non-arabica or robusta\/canephora cue: keep bitterness protection active/i, 'Cue non-arabika atau robusta/canephora: tetap aktifkan perlindungan pahit.'],
    [/Water needs caution or manual verification before treating the prediction as high confidence/i, 'Air perlu hati-hati atau verifikasi manual sebelum prediksi dianggap berkeyakinan tinggi.'],
    [/Grinder setting is estimated or fallback; calibrate by drawdown and taste/i, 'Setelan grinder masih berupa perkiraan; kalibrasi dari waktu air turun dan rasa.'],
    [/Adjust dose, water target, brewer size, or unsafe manual preset before brewing/i, 'Ubah dosis, target air, ukuran alat, atau preset manual yang tidak aman sebelum seduh.'],
    [/Brew the conservative baseline, then use taste feedback before changing dose or ratio/i, 'Gunakan acuan konservatif terlebih dahulu, lalu cek rasa sebelum mengubah dosis atau rasio.'],
    [/Use the balanced baseline, then record taste feedback after brewing/i, 'Gunakan acuan seimbang, lalu catat hasil evaluasi rasa setelah menyeduh.'],
    [/Brew the plan as a strong starting point; adjust only one variable after tasting/i, 'Gunakan rencana seduh ini sebagai titik awal yang kuat; ubah satu variabel saja setelah mencicipi.'],
    [/Add process, variety, roast development, or density to improve accuracy; taste feedback remains the first correction loop/i, 'Tambahkan proses, varietas, development sangrai, atau densitas untuk menaikkan akurasi; feedback rasa tetap jadi koreksi pertama.'],
    [/Water source: manual mineral entry/i, 'Sumber air: input mineral manual.'],
    [/Device profile source: (.+)/i, 'Sumber profil alat: $1.'],
    [/Brewer profile source: (.+)/i, 'Sumber profil alat: $1.'],
    [/Grinder setting source: (.+)/i, 'Sumber setting grinder: $1.'],
    [/Grinder source: (.+)/i, 'Sumber grinder: $1.'],
    [/Extraction complete\. Next step is finishing/i, 'Ekstraksi selesai. Langkah berikutnya adalah tahap penyelesaian.'],
    [/Fallback grinder lowers confidence; validate with drawdown and taste/i, 'Acuan grinder pengganti menurunkan keyakinan; kalibrasi dari waktu air turun dan rasa.'],
    [/Using (.+) family fallback profile/i, 'Menggunakan profil keluarga $1 sebagai pengganti.'],
    [/(.+?) was selected as the brand baseline, then adjusted manually/i, '$1 dipakai sebagai acuan merek, lalu disesuaikan secara manual.'],
    [/(.+?) brand water profile is active for this brew plan/i, 'Profil air merek $1 digunakan untuk rencana seduh ini.'],
    [/(.+?) does not have a full autofill panel in this catalog version/i, '$1 belum memiliki data isi otomatis yang lengkap pada versi katalog ini.'],
    [/(.+?) minerals were estimated from the water classification baseline/i, 'Mineral $1 diperkirakan dari acuan klasifikasi air.'],
    [/Device profile was generated from the (.+) family template for (.+)/i, 'Profil alat dibuat dari pola keluarga $1 untuk $2.'],
    [/Exact device profile matched: (.+)/i, 'Profil alat presisi ditemukan: $1.'],
    [/Operator knowledge active: (\d+) matched note\(s\) from the operator knowledge layer/i, 'Knowledge operator aktif: $1 catatan cocok dari layer knowledge operator.'],
    [/AeroPress service floor protects medium and lighter roasts from under-extraction; preheat, then press steadily instead of using a very low kettle temperature/i, 'AeroPress dinaikkan ke suhu aman agar roast medium atau lebih ringan tidak kurang ekstraksi; panaskan ruang seduh, lalu tekan stabil.'],
    [/French Press temperature kept in a calm immersion band so body builds without extracting harsh fines/i, 'Suhu French Press dijaga di rentang immersion yang tenang agar body terbentuk tanpa menarik fines pahit.'],
    [/Delicate Geisha\/Gesha iced profile capped at 92-94.*C to protect floral aroma while keeping Japanese-style concentration/i, 'Profil Geisha/Gesha es dibatasi 92-94\u00b0C agar aroma floral tetap aman sambil menjaga konsentrat Japanese style.'],
    [/Structured Indonesian iced profile uses lower kettle energy so body stays sweet instead of bitter or burnt/i, 'Profil es Indonesia yang body-nya tebal memakai energi kettle lebih rendah agar tetap manis, tidak pahit atau gosong.'],
    [/Washed Colombia on April iced gets a little more kettle energy to lift caramel, red-apple, and citrus sweetness/i, 'Washed Colombia pada April iced diberi sedikit energi kettle tambahan untuk mengangkat caramel, red apple, dan citrus sweetness.'],
    [/Washed Kenya light\/medium-light profile lifted toward 94-96.*C so berry, citrus, and bergamot notes extract clearly/i, 'Washed Kenya light/medium-light diarahkan ke 94-96\u00b0C agar berry, citrus, dan bergamot keluar jelas.'],
    [/Washed Ethiopia\/Yirgacheffe light profile lifted toward 94-96.*C to open citrus, honey, and floral clarity/i, 'Washed Ethiopia/Yirgacheffe light diarahkan ke 94-96\u00b0C untuk membuka citrus, honey, dan floral clarity.'],
    [/Washed Central America light\/medium-light profile nudged toward 93-95.*C for citrus, caramel, and clean sweetness/i, 'Washed Central America light/medium-light diarahkan ke 93-95\u00b0C untuk citrus, caramel, dan clean sweetness.'],
    [/Bright washed highland profile uses a slightly warmer filter envelope so clarity does not turn thin/i, 'Profil washed highland yang bright memakai envelope sedikit lebih panas agar clarity tidak terasa tipis.'],
    [/Barista temperature calibration active: (.+)/i, 'Kalibrasi suhu barista aktif: $1.'],
    [/Water source: (.+) \((.+)\)/i, 'Sumber air: $1 ($2).'],
    [/Knowledge v1 - Gayo: body cenderung tebal dengan aroma rempah yang kuat; jaga ekstraksi cukup rapi agar body tetap manis, tidak pahit, dan tidak keruh/i, 'Knowledge v1 - Gayo: body cenderung tebal dengan aroma rempah yang kuat; jaga ekstraksi cukup rapi agar body tetap manis, tidak pahit, dan tidak keruh.'],
    [/Knowledge v1 - V60: gunakan tuangan spiral atau center-to-mid secara konsisten untuk ekstraksi merata; hindari mengejar dinding filter pada fase akhir/i, 'Knowledge v1 - V60: gunakan tuangan spiral atau center-to-mid secara konsisten untuk ekstraksi merata; hindari mengejar dinding filter pada fase akhir.'],
    [/Brew (\d+(?:\.\d+)?) ml hot over (\d+(?:\.\d+)?) ml\/g ice \((.+)\)\. Final ratio is 1:(\d+(?:\.\d+)?); hot concentrate extracts at 1:(\d+(?:\.\d+)?)\. Keep pours compact to hold sweetness and clarity/i, 'Seduh $1 ml air panas di atas $2 ml/g es ($3). Rasio final 1:$4; konsentrat panas terekstraksi di 1:$5. Jaga tuangan tetap rapat untuk menjaga manis dan kejernihan.'],
    ...
    [/(.+?) target pushes sweetness before body/i, 'Target $1 mendorong rasa manis sebelum body.'],
    [/(.+?) target favors body with bitterness protection/i, 'Target $1 menaikkan body dengan perlindungan pahit.'],
    [/(.+?) target protects high-aroma fruit expression/i, 'Target $1 menjaga ekspresi buah beraroma tinggi.'],
    [/(.+?) target rounds acidity and keeps sweetness soft/i, 'Target $1 membulatkan keasaman dan menjaga manis tetap lembut.'],
    [/Balance target keeps acidity, sweetness, body, and clarity near baseline/i, 'Target seimbang menjaga keasaman, manis, body, dan kejernihan dekat baseline.'],
    [/Process (.+?) nudges (.+)/i, 'Proses $1 menggeser $2.'],
    [/Variety (.+?) nudges (.+)/i, 'Varietas $1 menggeser $2.'],
    [/(.+?) preset shapes Switch sweetness, clarity, body, and valve timing/i, 'Preset $1 membentuk manis, kejernihan, body, dan timing katup Switch.'],
    [/High-buffer water can mute acidity and floral clarity/i, 'Air buffer tinggi bisa meredam acidity dan kejernihan floral.'],
    [/Zero-mineral\/RO water should not be used without remineralization/i, 'Air nol mineral/RO sebaiknya tidak dipakai tanpa remineralisasi.'],
    [/Water minerals need manual verification before treating this profile as locked/i, 'Mineral air perlu verifikasi manual sebelum profil ini dianggap terkunci.'],
    [/Fallback grinder setting lowers confidence; validate with drawdown and taste/i, 'Setelan grinder fallback menurunkan keyakinan; validasi dengan air turun dan rasa.'],
    [/High-variability process needs taste feedback before stronger extraction changes/i, 'Proses variabilitas tinggi perlu feedback rasa sebelum perubahan ekstraksi yang lebih kuat.'],
    [/Workflow validation did not pass, so sensory prediction is not release-grade/i, 'Validasi panduan belum lolos, jadi prediksi rasa belum siap dipakai.'],
    [/Switch chamber validation is blocked; use the suggested safe programme before brewing/i, 'Validasi ruang Switch diblokir; gunakan program aman yang disarankan sebelum seduh.'],
    [/Switch chamber load is close to the safe limit; treat taste prediction as medium confidence/i, 'Muatan ruang Switch mendekati batas aman; anggap prediksi rasa sebagai keyakinan sedang.'],
    [/Switch preset cup profile is a curated prediction, not a guaranteed result/i, 'Profil rasa preset Switch adalah prediksi kurasi, bukan hasil yang dijamin.'],
    [/Valve closed\. Bloom (\d+(?:\.\d+)?) ml \((\d+(?:\.\d+)?)x dose\); pour gently and avoid aggressive swirling/i, 'Katup tertutup. Bloom $1 ml ($2x dosis); tuang lembut dan jangan swirl keras.'],
    [/Still closed\. Fill slowly to build body; open before it feels heavy or muddy/i, 'Masih tertutup. Isi perlahan untuk body tebal; buka sebelum terasa berat atau muddy.'],
    [/Still closed\. Fill the remaining water gently to promote sweetness and round body/i, 'Masih tertutup. Isi sisa air dengan tenang untuk sweetness dan body bulat.'],
    [/Hold closed contact time; open before flow stalls/i, 'Tahan kontak tertutup secukupnya; buka sebelum flow terasa stall.'],
    [/Open the valve cleanly and let the release drain without extra agitation/i, 'Buka katup sekali, lalu biarkan air turun tanpa aduk ulang.'],
    [/Swirl the server 5-8 seconds to integrate, then serve\. Note if body starts tasting muddy/i, 'Aduk server 5-8 detik lalu sajikan. Catat jika body mulai muddy.'],
    [/Swirl the server 5-8 seconds to integrate, then serve/i, 'Aduk server 5-8 detik lalu sajikan.'],
    [/Still closed\. Raise the chamber load to about (\d+(?:\.\d+)?) ml, then prepare for release/i, 'Masih tertutup. Naikkan muatan ruang sampai sekitar $1 ml, lalu siapkan release.'],
    [/Open the valve before the bed stalls; this keeps the finish clean/i, 'Buka katup sebelum bed melambat berat; ini menjaga finish tetap bersih.'],
    [/Valve open\. Complete the target with a steady center-to-mid pour, avoiding heavy wall-rinse/i, 'Katup terbuka. Selesaikan target dengan jalur pusat-ke-tengah, tanpa wall-rinse berat.'],
    [/Allow the drawdown to complete naturally; do not add extra water/i, 'Biarkan drawdown selesai natural; jangan tambah air di luar target.'],
    [/Valve open\. Bloom cleanly with (\d+(?:\.\d+)?) ml; keep flow low and avoid wall-rinse/i, 'Katup terbuka. Bloom bersih $1 ml; jaga flow rendah dan hindari wall-rinse.'],
    [/Valve open\. Build clarity with a steady stream and minimal agitation/i, 'Tetap terbuka. Bangun clarity dengan aliran stabil dan agitasi minimal.'],
    [/Close the valve briefly to capture late sweetness \((\d+(?:\.\d+)?)\s*ml\)\. Do not hold too long/i, 'Tutup singkat hanya untuk capture sweetness akhir ($1 ml). Jangan tahan terlalu lama.'],
    [/Open the valve early to keep the aroma, acidity, and clarity from being muted by body/i, 'Buka katup lebih awal agar aroma, acidity, dan clarity tidak tertutup body.'],
    [/Swirl the server gently and serve/i, 'Aduk server singkat lalu sajikan.'],
    [/Valve open from the start\. Bloom (\d+(?:\.\d+)?) ml; this is V60 mode, not immersion/i, 'Katup terbuka dari awal. Bloom $1 ml; ini Mode V60, bukan immersion.'],
    [/Keep the valve open and maintain a steady flow to protect clarity/i, 'Jaga katup tetap terbuka dan flow stabil untuk clarity.'],
    [/Complete the target volume with a clean path; do not close the valve/i, 'Selesaikan target dengan jalur bersih; jangan tutup katup.'],
    [/Drawdown naturally; no release checkpoint since the valve is already open/i, 'Drawdown natural; tidak ada titik release karena katup sudah terbuka.'],
    [/Valve closed\. Bloom the hot concentrate with (\d+(?:\.\d+)?) ml; ice is pre-weighed in the server/i, 'Katup tertutup. Bloom konsentrat panas $1 ml; es sudah ditimbang di server.'],
    [/Still closed\. Fill the hot concentrate target up to about (\d+(?:\.\d+)?) ml; do not add hidden bypass/i, 'Masih tertutup. Isi target panas tertutup sampai sekitar $1 ml; jangan tambah bypass tersembunyi.'],
    [/Open the valve and release the hot concentrate directly over the ice in the server/i, 'Buka katup dan release konsentrat panas langsung ke es di server.'],
    [/Valve open\. Pour only up to the hot water target; final volume is hot water \+ ice/i, 'Katup terbuka. Lanjutkan hanya sampai target air panas; final beverage tetap air panas + es.'],
    [/Stir the server 5-8 seconds to integrate concentrate and ice thoroughly/i, 'Aduk 5-8 detik agar konsentrat dan es menyatu.'],
    [/(.+?): MUGEN low-bypass is used with a cleaner open phase to boost clarity without exceeding Mugen capacity/i, '$1: MUGEN low-bypass dipakai dengan fase terbuka lebih bersih agar clarity naik tanpa meniru kapasitas Switch 03.'],
    [/(.+?): MUGEN low-bypass provides focus and body, keeping the 200 ml chamber safe with a conservative hybrid/i, '$1: MUGEN low-bypass memberi fokus dan body, tetapi chamber 200 ml dijaga dengan hybrid konservatif.'],
    [/(.+?): MUGEN low-bypass gives focused sweetness, with a safe hybrid keeping the 200 ml chamber controlled/i, '$1: MUGEN low-bypass memberi sweetness terfokus; hybrid aman menjaga chamber 200 ml tetap terkendali.'],
    [/(.+?): valve open from the start to maximize clarity and transparency/i, '$1: katup terbuka dari awal agar clarity dan transparansi lebih tinggi.'],
    [/(.+?): short closed phase to prevent acidity, aroma, and clarity from being masked by body/i, '$1: fase tertutup dibuat pendek supaya acidity, aroma, dan clarity tidak tertutup body.'],
    [/(.+?): larger bloom and gentle closed contact capturing sweetness without harsh swirling/i, '$1: bloom lebih besar dan kontak tertutup lembut menangkap sweetness tanpa swirl keras.'],
    [/(.+?): longer closed contact to enhance body, keeping chamber loads safe to avoid muddy notes/i, '$1: kontak tertutup lebih panjang menaikkan body, tetapi tetap dibatasi chamber agar tidak muddy.'],
    [/(.+?): medium closed capture preserving fruit sweetness, followed by an open finish to keep it clean/i, '$1: closed capture sedang menjaga fruit sweetness, lalu open finish menjaga bersih.'],
    [/(.+?): closed bloom capturing early sweetness, followed by an open finish to maintain a clean cup/i, '$1: closed bloom menangkap sweetness awal, lalu open finish menjaga clean finish.'],
    [/Full immersion requires closed chamber load of (\d+(?:\.\d+)?) ml, exceeding the safe limit of (\d+(?:\.\d+)?) ml/i, 'Full immersion butuh muatan ruang tertutup $1 ml, melebihi batas aman $2 ml.'],
    [/Keep the closed valve phase at (\d+(?:\.\d+)?) ml or lower; finish with the valve open/i, 'Jaga fase katup tertutup di $1 ml atau lebih rendah; selesaikan dengan katup terbuka.'],
    [/High-buffer water can mute bright notes; open the valve earlier if the cup tastes flat/i, 'Air buffer tinggi bisa meredam rasa cerah; buka katup lebih awal jika cangkir terasa datar.'],
    [/Grinder reference has low confidence; validate the drawdown before shifting recipe numbers/i, 'Referensi grinder masih rendah keyakinan; validasi air turun sebelum mengubah angka resep.'],
    [/Manual preset is less aligned with bright\/floral targets; clarity can drop\. Use Hybrid Bright Clean or V60 Mode for more transparency/i, 'Preset manual kurang selaras dengan target cerah/floral; kejernihan bisa turun. Gunakan Hybrid Bright Clean atau Mode V60 jika ingin lebih transparan.'],
    [/Manual preset is cleaner than the body target; body can taste lighter\. Use Heavy Body only if the chamber capacity is safe/i, 'Preset manual lebih bersih daripada target body; body bisa lebih ringan. Gunakan Heavy Body hanya jika muatan ruang aman.'],
    [/Manual V60 mode is more transparent than the sweet\/round target; sweetness can taste lighter/i, 'Mode V60 manual lebih transparan daripada target manis/bulat; rasa manis bisa lebih ringan.'],
    [/Peak closed chamber load of (\d+(?:\.\d+)?) ml exceeds the safe limit of (\d+(?:\.\d+)?) ml\. Use Switch 03, V60 mode, or a conservative hybrid/i, 'Puncak muatan ruang tertutup $1 ml melebihi batas aman $2 ml. Gunakan Switch 03, Mode V60, atau hybrid konservatif.'],
    [/Peak closed chamber load of (\d+(?:\.\d+)?) ml is close to the safe limit of (\d+(?:\.\d+)?) ml\. Open the valve before the bed stalls/i, 'Puncak muatan ruang tertutup $1 ml mendekati batas aman $2 ml. Buka katup sebelum bed mulai macet.'],
    [/Peak closed chamber load of (\d+(?:\.\d+)?) ml is safe below the limit of (\d+(?:\.\d+)?) ml/i, 'Puncak muatan ruang tertutup $1 ml masih aman di bawah batas $2 ml.'],
    [/No dose matrix row found for this dose in (.+?); use as a conservative starting point/i, '$1 belum punya baris dose matrix untuk dosis ini; gunakan sebagai titik awal konservatif.'],
    [/(.+?) is not safe for (.+?) (.+?): closed chamber load exceeds the safe limit of (\d+(?:\.\d+)?) ml/i, '$1 tidak aman untuk $2 $3: muatan ruang tertutup melewati batas aman $4 ml.'],
    [/(.+?) can be used for (.+?) (.+?), but keep chamber load below (\d+(?:\.\d+)?) ml and open earlier if the flow slows/i, '$1 bisa dipakai untuk $2 $3, tetapi jaga muatan ruang di bawah $4 ml dan buka lebih awal jika flow melambat.'],
    [/(.+?) is safe for (.+?) (.+?); chamber load limit is (\d+(?:\.\d+)?) ml/i, '$1 aman untuk $2 $3; batas muatan ruang $4 ml.'],
    [/Official (.+) reference baseline for pour over style brewing; adjust slightly finer for iced and slightly coarser for larger flat-bottom beds only after taste/i, 'Baseline referensi resmi $1 untuk seduh filter; geser sedikit lebih halus untuk iced dan sedikit lebih kasar untuk flat-bottom besar hanya setelah cek rasa.'],
    [/Finish calmly and let the drawdown stay tidy/i, 'Akhiri dengan tenang dan biarkan air turun tetap rapi.'],
    [/Stop adding water here\. Let the bed finish draining over the measured ice, then stir the server 5-8 seconds before serving/i, 'Berhenti tambah air di sini. Biarkan bed selesai turun di atas es terukur, lalu aduk server 5-8 detik sebelum disajikan.'],
    [/Let drawdown finish over the measured ice; stir the server after the final drips so service stays separate from brewing/i, 'Biarkan air turun selesai di atas es terukur; aduk server setelah tetesan akhir agar tahap saji tetap terpisah dari seduh.'],
    [/Target (\d+(?:\.\d+)?) ml hot water\. Land the final hot-water target only; the ice is intentional bypass, not another pour through the bed/i, 'Target $1 ml air panas. Capai target air panas terakhir saja; es adalah bypass terukur, bukan tuangan tambahan lewat bed.'],
    [/Land the final hot-water target only; the ice is intentional bypass, not another pour through the bed/i, 'Capai target air panas terakhir saja; es adalah bypass terukur, bukan tuangan tambahan lewat bed.'],
    [/Open the bloom evenly and let the cone drain cleanly before the next pour/i, 'Buka bloom secara merata lalu biarkan cone turun bersih sebelum tuangan berikutnya.'],
    [/Push a clean center-to-mid pour and keep the cone walls quiet/i, 'Dorong tuangan bersih dari tengah ke area tengah-luar dan jaga dinding cone tetap tenang.'],
    [/Keep the later V60 phase centered so the cup stays transparent/i, 'Jaga fase akhir V60 tetap di tengah agar cangkir tetap jernih.'],
    [/Finish calmly and let the cone drain without chasing the walls/i, 'Akhiri dengan tenang dan biarkan cone turun tanpa mengejar dinding.'],
    [/Keep the bloom light and even so the faster cone flow stays clean/i, 'Jaga bloom tetap ringan dan merata agar aliran cone yang lebih cepat tetap bersih.'],
    [/Use compact pulses and keep the flow agile through the middle/i, 'Gunakan pulse yang rapat dan jaga aliran tetap lincah di fase tengah.'],
    [/Hold the later middle short and tidy so transparency stays high/i, 'Jaga fase tengah akhir tetap singkat dan rapi agar kejernihan tetap tinggi.'],
    [/Close with a light finishing pour and let the fast drawdown stay clean/i, 'Tutup dengan tuangan akhir yang ringan dan biarkan air turun cepat tetap bersih.'],
    [/Keep the bloom centered and slightly deeper to establish a sweet core/i, 'Jaga bloom tetap di tengah dan sedikit lebih dalam untuk membangun inti rasa manis.'],
    [/Hold a tighter center pour to build sweetness with stable contact time/i, 'Pertahankan tuangan tengah yang lebih rapat untuk membangun manis dengan waktu kontak yang stabil.'],
    [/Carry the late middle with a narrow, steady stream/i, 'Lanjutkan fase tengah akhir dengan aliran sempit dan stabil.'],
    [/Finish narrow and controlled so sweetness stays intact/i, 'Akhiri dengan aliran sempit dan terkontrol agar rasa manis tetap utuh.'],
    [/Wet the flat bed edge to edge, then let it settle level before building the cup/i, 'Basahi flat bed dari tepi ke tepi lalu biarkan rata sebelum membangun cangkir.'],
    [/Keep the flat bed level with even pulses from center to edge/i, 'Jaga flat bed tetap rata dengan pulse merata dari tengah ke tepi.'],
    [/Protect the later middle with flat, even contact across the bed/i, 'Jaga fase tengah akhir dengan kontak yang rata di seluruh bed.'],
    [/Land the final water evenly to keep drawdown flat and tidy/i, 'Letakkan air terakhir secara merata agar air turun tetap rata dan rapi.'],
    [/Open the trapezoid bed evenly so the first drain starts clean and forgiving/i, 'Buka bed trapezoid secara merata agar aliran pertama mulai bersih dan mudah dikendalikan.'],
    [/Keep the middle pours level and measured so the flow stays forgiving/i, 'Jaga tuangan tengah tetap rata dan terukur agar aliran tetap mudah dikendalikan.'],
    [/Carry the later middle with stable, level contact/i, 'Lanjutkan fase tengah akhir dengan kontak yang stabil dan rata.'],
    [/Finish with a tidy, level pour and let the bed drain cleanly/i, 'Akhiri dengan tuangan yang rapi dan rata lalu biarkan bed turun bersih.'],
    [/Use a short, even bloom and avoid excess swirl so the flat bed stays fast/i, 'Gunakan bloom singkat dan merata lalu hindari swirl berlebih agar flat bed tetap cepat.'],
    [/Keep the late pulses quick and even so the cup stays open/i, 'Jaga pulse akhir tetap cepat dan merata agar cangkir tetap terbuka.'],
    [/Finish early and clean; avoid stretching the last phase/i, 'Akhiri lebih cepat dan bersih; hindari memanjangkan fase terakhir.'],
    [/Fully wet the thick filter path and the coffee bed before pushing the next pour/i, 'Basahi jalur filter tebal dan bed kopi sepenuhnya sebelum tuangan berikutnya.'],
    [/Use a steady stream and let the thick filter manage the flow/i, 'Gunakan aliran yang stabil dan biarkan filter tebal mengatur flow.'],
    [/Keep the later middle open and avoid flooding the filter wall/i, 'Jaga fase tengah akhir tetap terbuka dan hindari membanjiri dinding filter.'],
    [/Finish before the filter stalls and let the drawdown stay open/i, 'Akhiri sebelum filter tersendat dan biarkan air turun tetap terbuka.'],
    [/Saturate the full bed evenly and let immersion start building sweetness/i, 'Saturasi seluruh bed secara merata lalu biarkan immersion mulai membangun rasa manis.'],
    [/Use the middle phase to build immersion gently rather than chasing more turbulence/i, 'Gunakan fase tengah untuk membangun immersion dengan tenang, bukan mengejar turbulensi tambahan.'],
    [/Keep the later immersion phase quiet so the release stays clean/i, 'Jaga fase immersion akhir tetap tenang agar buka katup tetap bersih.'],
    [/Open the release cleanly and let the bed drain without stirring the finish/i, 'Buka katup dengan bersih lalu biarkan bed turun tanpa mengaduk fase akhir.'],
    [/Assemble inverted safely and saturate the coffee bed quickly/i, 'Rakit posisi inverted dengan aman dan basahi kopi dengan cepat.'],
    [/Use a stable inverted setup, add water decisively to cover all grounds, and keep the chamber upright-ready/i, 'Gunakan posisi inverted yang stabil, isi air dengan mantap sampai seluruh kopi basah, dan pastikan chamber siap dibalik.'],
    [/Brew a compact concentrate and keep bypass water separate/i, 'Seduh konsentrat kental yang padat dan pisahkan air bypass.'],
    [/Pour hot water to wet the high-dose bed cleanly; keep bypass water reserved for post-press dilution/i, 'Isi air panas untuk membasahi kopi dosis tinggi; simpan air bypass untuk ditambahkan setelah kopi selesai ditekan.'],
    [/Saturate the full-chamber upright bed edge to edge/i, 'Basahi seluruh kopi di chamber tegak secara merata.'],
    [/Add water decisively across the slightly coarser bed; no post-brew bypass will be used/i, 'Isi air dengan mantap ke seluruh kopi yang digiling sedikit lebih kasar; resep ini tidak memakai bypass.'],
    [/Pour swiftly in circles over double rinsed paper filters/i, 'Isi air dengan cepat di atas filter kertas ganda yang sudah dibilas.'],
    [/Use double rinsed paper filters, add water swiftly to wet grounds quickly, and establish high clarity/i, 'Gunakan filter kertas ganda yang sudah dibilas, isi air dengan cepat agar kopi lekas basah, dan jaga hasil tetap bersih.'],
    [/Saturate the fine grounds cleanly and prepare for agitation/i, 'Basahi kopi gilingan halus secara merata dan bersiap untuk adukan.'],
    [/Wet the fine grounds thoroughly, ensuring complete saturation to open high sweet solubility/i, 'Basahi kopi gilingan halus dengan menyeluruh agar kelarutan rasa manis yang tinggi terekstraksi maksimal.'],
    [/Saturate the compact upright bed quickly and evenly/i, 'Basahi bed kopi tegak yang padat secara cepat dan merata.'],
    [/Add water decisively to cover all dry grounds, and let the initial wetting lock down uniform contact/i, 'Isi air dengan mantap sampai seluruh bagian kering basah, lalu biarkan kontak ekstraksi mulai merata.'],
    [/Stir 4 times to wet grounds, then prepare to seal/i, 'Aduk 4 kali untuk membasahi kopi, lalu bersiap untuk menyegel.'],
    [/Stir thoroughly to integrate all grounds, ensuring complete contact before the flip phase/i, 'Aduk rata agar seluruh kopi terendam merata sebelum fase pembalikan.'],
    [/Stir vigorously 4-5 times to maximize early extraction/i, 'Aduk kuat 4-5 kali untuk memaksimalkan ekstraksi awal.'],
    [/Use vigorous stirring to push solubility in the compact concentrate, building a sweet, heavy core/i, 'Aduk dengan kuat untuk meningkatkan kelarutan pada konsentrat padat, membangun rasa manis inti yang tebal.'],
    [/Stir gently 2-3 times to settle the full slurry/i, 'Aduk perlahan 2-3 kali untuk meratakan campuran kopi.'],
    [/Agitate gently to distribute water throughout the deep slurry, avoiding heavy bottom-packing/i, 'Aduk lembut agar air tersebar merata dan kopi tidak menumpuk terlalu padat di dasar chamber.'],
    [/Stir gently exactly 2 times to prevent fines migration/i, 'Aduk perlahan tepat 2 kali untuk mencegah perpindahan partikel halus.'],
    [/Agitate minimally to protect clarity; do not sweep the sides or migrate fine particles/i, 'Lakukan agitasi minimal untuk melindungi kejernihan; jangan mengusap dinding kertas atau memicu perpindahan fines.'],
    [/Stir vigorously 5-6 times to build heavy body/i, 'Aduk kuat 5-6 kali untuk membangun body yang tebal.'],
    [/Use strong circular stirring to build texture and dissolve sweet syrupy compounds/i, 'Aduk memutar dengan kuat untuk membentuk tekstur rasa dan melarutkan senyawa manis yang kental.'],
    [/Stir gently 3 times, then let contact settle/i, 'Aduk perlahan 3 kali, lalu biarkan kontak ekstraksi merata.'],
    [/Agitate in calm circular paths to integrate the slurry, then let contact time build sweet extraction/i, 'Lakukan adukan melingkar yang tenang untuk menyatukan slurry, lalu biarkan waktu kontak membentuk ekstraksi manis.'],
    [/Screw filter cap tightly and prepare to flip securely/i, 'Kencangkan filter cap dan bersiap untuk membalikkan dengan aman.'],
    [/Seal the pre-wet cap, let the inverted slurry steep, and prepare for a decisive flip/i, 'Kunci cap yang sudah dibilas, biarkan kopi dalam posisi inverted terendam tenang, lalu bersiap membalik alat dengan mantap.'],
    [/Let concentrate steep briefly before the press/i, 'Biarkan konsentrat terendam singkat sebelum ditekan.'],
    [/Maintain compact, high-solubility contact; prepare separate dilution water for later/i, 'Pertahankan kontak kelarutan tinggi yang padat; siapkan air dilusi terpisah untuk langkah akhir.'],
    [/Seal with the plunger and allow a longer immersion steep/i, 'Segel dengan plunger dan biarkan rendaman (immersion) berlangsung lebih lama.'],
    [/Insert the plunger slightly to hold the full-chamber volume; let immersion work longer for sweet depth/i, 'Pasang plunger sedikit untuk menahan volume penuh chamber; biarkan immersion bekerja lebih lama untuk kedalaman rasa manis.'],
    [/Cap to seal and keep the steep window short/i, 'Pasang plunger untuk menyegel dan jaga waktu rendam tetap singkat.'],
    [/Create a vacuum seal quickly, and limit immersion contact time to protect bright acidity/i, 'Buat segel vakum dengan cepat, dan batasi waktu kontak rendam untuk melindungi acidity yang cerah.'],
    [/Seal with the plunger and allow a longer sweet-body immersion/i, 'Segel dengan plunger dan biarkan rendam lebih lama untuk body yang manis.'],
    [/Insert the plunger and extend the steep window so deep caramel and chocolate solubility completes/i, 'Pasang plunger sedikit dan perpanjang waktu rendam agar ekstraksi senyawa karamel dan cokelat yang manis tuntas.'],
    [/Insert the plunger slightly to create a vacuum seal/i, 'Pasang plunger sedikit untuk membuat segel vakum.'],
    [/Create a vacuum seal to prevent premature dripping; steep calmly to hold clear flavor/i, 'Buat segel vakum untuk mencegah kopi menetes sebelum waktunya; rendam tenang untuk menjaga rasa tetap jernih.'],
    [/Flip decisively and press slowly down to the grounds/i, 'Balikkan secara mantap lalu tekan perlahan hingga menyentuh bed kopi.'],
    [/Flip onto the server in one smooth motion, press steadily, and stop before forcing the dry hiss/i, 'Balikkan ke atas cangkir dalam satu gerakan mulus, tekan dengan stabil, dan berhenti sebelum desis kering.'],
    [/Press steadily, stop before the hiss, and dilute/i, 'Tekan secara stabil, berhenti sebelum desis, lalu encerkan.'],
    [/Press the concentrate steadily, stop strictly before the hiss, then dilute with reserved bypass water/i, 'Tekan konsentrat kental secara stabil, berhenti tepat sebelum desis, lalu encerkan dengan air bypass terpisah.'],
    [/Remove plunger, press steadily, and drain fully/i, 'Lepas plunger, tekan dengan stabil, dan berhenti sebelum desis kering.'],
    [/Uncap, press slowly over 30-40 seconds, and let the full chamber volume drain completely/i, 'Buka plunger, tekan perlahan selama 25-35 detik, dan berhenti sebelum desis kering.'],
    [/Press extremely slowly with minimal force, stopping before the hiss/i, 'Tekan sangat perlahan dengan tenaga minimal, berhenti sebelum desis.'],
    [/Press with feather-light pressure over 35-40 seconds, and stop strictly before the hiss to keep lipids locked/i, 'Tekan sangat ringan selama 20-30 detik dan berhenti sebelum desis agar hasil tetap bersih.'],
    [/Press slowly to the very bottom of the puck/i, 'Tekan perlahan mendekati desis, lalu hentikan sebelum rasa menjadi kering.'],
    [/Apply firm, stable force down to the absolute bottom of the bed to extract sweet soluble layers/i, 'Gunakan tekanan stabil yang pelan; dekati desis hanya untuk menambah tekstur, bukan untuk memaksa ampas halus masuk ke cangkir.'],
    [/Press steadily and stop at the first hiss/i, 'Tekan secara stabil dan berhenti tepat pada desis pertama.'],
    [/Press steadily over 25-35 seconds, and stop strictly at the first hiss to protect clarity/i, 'Tekan secara stabil selama 25-35 detik, dan berhenti tepat pada desis pertama untuk melindungi kejernihan rasa.'],
    [/Pour boiling water swiftly to saturate all coffee grounds/i, 'Tuang air mendidih dengan cepat untuk membasahi seluruh bubuk kopi.'],
    [/Pour boiling water swiftly to wet all grounds cleanly and establish a stable heat retention in the glass chamber/i, 'Tuang air mendidih dengan cepat untuk membasahi kopi secara bersih dan menjaga retensi panas yang stabil pada glass chamber.'],
    [/Pour water in circular motions to saturate the medium-ground bed/i, 'Tuang air dengan gerakan melingkar untuk membasahi bed kopi gilingan sedang.'],
    [/Pour water gently in slow circular paths to wet the medium-ground bed, promoting even wetting before the clean steep phase/i, 'Tuang air secara perlahan dengan gerakan memutar lembut untuk membasahi bed kopi gilingan sedang secara merata sebelum rendaman bersih.'],
    [/Pour water rapidly over the heavy dose to wet the fine grounds/i, 'Tuang air secara cepat ke kopi dosis tinggi untuk membasahi gilingan halus.'],
    [/Pour hot water rapidly to wet the high-dose bed; maintain maximum thermal mass inside the chamber/i, 'Tuang air panas secara cepat untuk membasahi bed dosis tinggi; pertahankan massa termal maksimal di dalam chamber.'],
    [/Pour water gently to promote a round, sweet extraction/i, 'Tuang air secara lembut untuk memicu ekstraksi rasa manis yang bulat.'],
    [/Pour water gently at a slightly lower temperature to promote high sweet solubility without dissolving bitter compounds/i, 'Tuang air perlahan pada suhu sedikit lebih rendah untuk meningkatkan kelarutan manis yang tinggi tanpa mengekstraksi senyawa pahit.'],
    [/Allow the crust to form undisturbed for 4 minutes/i, 'Biarkan kerak kopi (crust) terbentuk tenang tanpa gangguan selama 4 menit.'],
    [/Leave the chamber undisturbed while a thick, aromatic crust of coffee grounds forms at the surface/i, 'Biarkan chamber tenang tanpa gangguan selagi kerak kopi (crust) yang kental dan aromatik terbentuk di permukaan.'],
    [/Steep cleanly while the double filter elements are prepared/i, 'Rendam tenang selagi elemen penyaring ganda disiapkan.'],
    [/Allow full immersion to proceed undisturbed; prepare the double mesh or paper filter insert by pre-wetting with hot water/i, 'Biarkan immersion penuh berlangsung tanpa gangguan; siapkan double mesh atau kertas saring dengan membilasnya terlebih dahulu.'],
    [/Stir vigorously 5-6 times to maximize early extraction strength/i, 'Aduk dengan kuat 5-6 kali untuk memaksimalkan kekuatan ekstraksi awal.'],
    [/Use strong agitation early to break down the dense slurry, maximizing extraction from the concentrated bed/i, 'Lakukan agitasi kuat di awal untuk memecah slurry yang padat, memaksimalkan kekuatan ekstraksi dari bed konsentrat kental.'],
    [/Stir gently exactly 2 times to distribute extraction evenly/i, 'Aduk perlahan tepat 2 kali untuk mendistribusikan ekstraksi secara merata.'],
    [/Stir exactly twice with a light touch to distribute heat and grounds without introducing bitterness/i, 'Aduk tepat dua kali dengan sentuhan ringan untuk meratakan panas dan bubuk kopi tanpa menimbulkan rasa pahit.'],
    [/Stir the crust gently, skim the surface foam and floating oils/i, 'Aduk kerak kopi perlahan, buang busa permukaan dan minyak yang mengapung.'],
    [/Break the crust with 2-3 gentle folds, then skim the remaining light foam and floating oils from the surface to ensure high cup clarity/i, 'Pecahkan kerak (crust) dengan 2-3 adukan lipat lembut, lalu bersihkan sisa busa dan minyak mengapung dari permukaan untuk kejernihan rasa tinggi.'],
    [/Give a light swirl to settle the grounds before placing the plunger/i, 'Goyang memutar (swirl) ringan agar bubuk kopi turun sebelum plunger dipasang.'],
    [/Give a light, gentle swirl to detach grounds from the glass wall and let them settle to the bottom/i, 'Goyang memutar (swirl) lembut untuk melepaskan bubuk kopi dari dinding kaca dan biarkan mengendap ke dasar.'],
    [/Let the thick immersion concentrate develop body and richness/i, 'Biarkan konsentrat rendam yang kental mengembangkan body dan kekayaan rasa.'],
    [/Allow the high-strength immersion slurry to steep, building a syrupy mouthfeel and sweet cocoa structure/i, 'Biarkan slurry immersion berkekuatan tinggi terendam tenang, membentuk mouthfeel seperti sirup dan struktur rasa cokelat manis.'],
    [/Steep quietly to allow sugar compounds to fully dissolve/i, 'Rendam tenang agar senyawa gula terlarut sempurna.'],
    [/Steep quietly; the lower temperature protects sweet caramel and chocolate solubility/i, 'Rendam tenang; suhu yang lebih rendah melindungi kelarutan senyawa manis karamel dan cokelat.'],
    [/Fit plunger and lower it just to touch the liquid surface; decant gently/i, 'Pasang plunger dan turunkan mesh hanya sampai menyentuh permukaan kopi; tuang pisah perlahan.'],
    [/Fit the plunger and lower the mesh just to touch the surface \(do not plunge!\)\. Pour out extremely slowly to prevent churning the settled bed/i, 'Pasang plunger dan turunkan mesh hanya sampai batas permukaan kopi (jangan ditekan!). Tuang pisah sangat perlahan agar bubuk kopi dasar tidak keruh.'],
    [/Press down slowly over 30 seconds through the double filter; serve cleanly/i, 'Tekan perlahan selama 30 detik melalui filter ganda; sajikan bersih.'],
    [/Press the double filter down slowly with uniform, light force over 30 seconds, trapping all fines for an ultra-clean finish/i, 'Tekan filter ganda secara perlahan dengan tenaga ringan yang seragam selama 30 detik, menyaring seluruh ampas halus demi hasil akhir yang super bersih.'],
    [/Press firmly to the bottom of the puck, serve as concentrate or bypass/i, 'Tekan mantap hingga ke dasar bed kopi, sajikan sebagai konsentrat kental atau tambahkan bypass.'],
    [/Apply firm, stable force down to the absolute bottom of the coffee puck to extract sweet soluble layers; serve as concentrate or dilute with bypass/i, 'Berikan tenaga penekanan stabil hingga ke dasar bed kopi untuk memeras sisa larutan manis; sajikan kental atau encerkan dengan bypass.'],
    [/Plunge extremely slowly over 30 seconds to avoid fines churning/i, 'Tekan plunger sangat perlahan selama 30 detik untuk menghindari turbulensi partikel halus.'],
    [/Plunge with feather-light force to avoid fines migration, and decant immediately to stop the extraction/i, 'Plunge dengan tenaga sangat ringan untuk menghindari perpindahan ampas halus, lalu tuang pisah segera untuk menghentikan ekstraksi.'],
    ...
    [/Pakai tiga pulsa rendah dari tengah agar permukaan tetap rata/i, 'Use three gentle center pulses to keep the coffee bed level.'],
    [/Jaga pulsa seragam; alat alas datar bekerja paling stabil saat permukaan tidak bergelombang/i, 'Keep each pulse even; a flat-bottom brewer is most stable when the coffee bed stays level.'],
    ...
    [/Tuang empat pulsa pendek; setiap pulsa masuk sebelum permukaan terlalu kering/i, 'Pour four short pulses; each pulse goes in before the surface gets too dry.'],
    [/Jaga tempo cepat tetapi tidak kasar agar hamparan kopi tetap rata/i, 'Maintain a fast but gentle tempo to keep the coffee bed level.'],
    [/Bilas filter berlipat, panaskan alat, dan siapkan aliran kecil yang konsisten/i, 'Rinse the wave filter, preheat the brewer, and set up a consistent small stream.'],
    [/Tuang kontinu lambat dari tengah; biarkan alat alas datar menjaga sebaran air/i, 'Pour a slow, continuous stream from the center; let the flat-bottom brewer maintain water distribution.'],
    [/Pertahankan tinggi air rendah supaya ekstraksi tidak berat sebelah/i, 'Keep the water level low so extraction does not skew unevenly.'],
    [/Biarkan air turun alami sampai waktu akhir/i, 'Let the water draw down naturally until the final time.'],
    [/Sajikan setelah permukaan selesai turun/i, 'Serve after the surface finishes drawing down.'],
    [/Bilas filter berlipat, masukkan es ke wadah saji, lalu targetkan air panas saja/i, 'Rinse the wave filter, add ice to the server, then target only hot water.'],
    [/Tuang pulsa lebih rapat agar ekstraksi cukup sebelum kopi menyentuh es/i, 'Pour tighter pulses for sufficient extraction before the coffee hits the ice.'],
    [/Jaga hamparan kopi rata dan hentikan tuang tepat di target air panas/i, 'Keep the coffee bed level and stop pouring exactly at the hot water target.'],
    [/Biarkan tetesan akhir turun ke atas es tanpa air tambahan/i, 'Let the final drops fall onto the ice without extra water.'],
    [/Aduk es 5-8 detik sebelum diminum/i, 'Stir the ice for 5-8 seconds before drinking.'],
    [/Bilas filter berlipat, panaskan alat, dan ratakan dosis tinggi dengan lembut/i, 'Rinse the wave filter, preheat the brewer, and gently level the high dose.'],
    [/Tuang pendek dan terukur agar konsentrat tetap stabil/i, 'Pour short and measured pulses to keep the concentrate stable.'],
    [/Jaga aliran rendah; dosis tinggi butuh kontrol, bukan agitasi kasar/i, 'Keep the flow low; high dose needs control, not rough agitation.'],
    [/Biarkan air turun penuh sebelum disajikan/i, 'Let the water draw down completely before serving.'],
    [/Aduk hasil seduh pelan agar konsentrat merata/i, 'Gently stir the brew so the concentrate mixes evenly.'],
    [/Bilas filter, panaskan Clever dan wadah saji, lalu pastikan alas pelepas siap/i, 'Rinse the filter, preheat the Clever and serving vessel, then ensure the release base is ready.'],
    [/Masukkan air dan kopi di ruang tertutup, lalu basahi hamparan kopi merata/i, 'Add water and coffee in the closed chamber, then wet the coffee bed evenly.'],
    [/Rendam tenang; waktu kontak adalah kontrol utama/i, 'Steep calmly; contact time is the main control.'],
    [/Letakkan di atas wadah saji untuk membuka pelepas, lalu jangan aduk saat air turun/i, 'Place on the serving vessel to open the release, then do not stir during the drawdown.'],
    [/Sajikan setelah air turun bersih/i, 'Serve after the drawdown finishes cleanly.'],
    [/Bilas filter, panaskan Clever, lalu siapkan air lebih dulu di ruang seduh/i, 'Rinse the filter, preheat the Clever, then prepare water first in the brew chamber.'],
    [/Masukkan air dulu, taburkan kopi merata, lalu aduk ringan agar semua bubuk basah/i, 'Add water first, scatter the coffee evenly, then stir lightly so all grounds are wet.'],
    [/Rendam tanpa gangguan; gaya ini menekan gumpalan kering/i, 'Steep undisturbed; this style sinks dry clumps.'],
    [/Buka pelepas setelah rendam selesai dan biarkan aliran keluar bersih/i, 'Open the release after the steep finishes and let the flow drain cleanly.'],
    [/Sajikan setelah air turun penuh/i, 'Serve after the drawdown finishes completely.'],
    ...
    [/Tambahkan sisa air sesuai target sebelum pelepas dibuka/i, 'Add the remaining water to target before opening the release.'],
    [/Buka pelepas dan biarkan gaya gravitasi menyelesaikan fase turun/i, 'Open the release and let gravity finish the drawdown.'],
    [/Bilas filter, masukkan es ke wadah saji, dan siapkan Clever untuk air panas target/i, 'Rinse the filter, add ice to the server, and prepare the Clever for the hot water target.'],
    [/Rendam dengan air panas yang lebih sedikit agar konsentrat cukup kuat/i, 'Steep with less hot water so the concentrate is strong enough.'],
    [/Jaga waktu kontak; es hanya mendinginkan dan mengencerkan sesuai resep/i, 'Keep contact time steady; ice only cools and dilutes according to the recipe.'],
    [/Buka pelepas ke atas es dan berhenti di target air panas/i, 'Open the release over the ice and stop at the hot water target.'],
    [/Bilas filter, panaskan Clever, dan siapkan dosis tinggi tanpa memadatkan/i, 'Rinse the filter, preheat the Clever, and prepare the high dose without compressing.'],
    [/Masukkan air dan kopi dengan ruang aman agar rendaman tidak meluap/i, 'Add water and coffee with safe headroom so the steep does not overflow.'],
    [/Rendam stabil untuk konsentrat; jangan menambah agitasi kasar/i, 'Steep steadily for the concentrate; do not add rough agitation.'],
    [/Buka pelepas saat waktu tercapai dan biarkan aliran turun penuh/i, 'Open the release when time is reached and let the flow drain completely.'],
    ...
    [/Pasang filter siphon, isi air, dan panaskan sampai air naik penuh/i, 'Set the siphon filter, add water, and heat until the water fully rises.'],
    [/Biarkan air naik penuh sebelum menambahkan kopi/i, 'Let the water rise fully before adding coffee.'],
    [/Masukkan kopi saat ruang atas sudah panas, lalu aduk singkat/i, 'Add coffee when the upper chamber is hot, then stir briefly.'],
    [/Jaga kontak stabil di ruang atas tanpa adukan berulang/i, 'Keep contact stable in the upper chamber without repeated stirring.'],
    [/Matikan panas agar vakum menarik kopi turun alami/i, 'Turn off the heat so vacuum drawdown pulls the coffee down naturally.'],
    [/Sajikan segera setelah air turun bersih/i, 'Serve promptly after the drawdown finishes cleanly.'],
    [/Pasang filter siphon, panaskan air, dan siapkan tiga agitasi kecil yang terukur/i, 'Set the siphon filter, heat the water, and prepare three small measured agitations.'],
    [/Masukkan kopi saat air naik, lalu aduk pertama untuk pembasahan merata/i, 'Add coffee as the water rises, then make the first stir for even wetting.'],
    [/Gunakan agitasi kedua dan ketiga secara ringan, bukan mengocok ruang atas/i, 'Use the second and third agitations lightly; do not shake the upper chamber.'],
    [/Matikan panas tepat waktu agar air turun bersih/i, 'Turn off the heat on time so drawdown finishes cleanly.'],
    [/Sajikan setelah seduhan turun dan permukaan tenang/i, 'Serve after the brew draws down and the surface settles.'],
    [/Pasang filter siphon dan jaga panas lebih lembut untuk kopi halus/i, 'Set the siphon filter and keep gentler heat for delicate coffee.'],
    [/Masukkan kopi setelah air naik stabil, lalu aduk sangat singkat/i, 'Add coffee after the water rise stabilizes, then stir very briefly.'],
    [/Jaga kontak lebih tenang agar rasa ringan tidak tertutup body/i, 'Keep contact calmer so lighter flavors are not covered by body.'],
    [/Matikan panas sedikit lebih awal untuk fase turun yang lembut/i, 'Turn off the heat slightly earlier for a gentler drawdown phase.'],
    [/Pasang filter siphon, panaskan stabil, dan siapkan kontak singkat berenergi/i, 'Set the siphon filter, heat steadily, and prepare a short energetic contact phase.'],
    [/Masukkan kopi dan aduk mantap di awal/i, 'Add coffee and stir confidently at the start.'],
    [/Jaga kontak padat tetapi singkat supaya body terbentuk tanpa rasa rebus/i, 'Keep contact dense but short so body builds without boiled flavor.'],
    [/Matikan panas dan dorong fase turun cepat dengan vakum bersih/i, 'Turn off the heat and drive a fast drawdown with clean vacuum pull.'],
    [/Aduk hasil seduh pelan sebelum disajikan/i, 'Stir the brew gently before serving.'],
    [/Pasang filter siphon dan siapkan ruang atas bersih untuk infusi aromatik/i, 'Set the siphon filter and prepare a clean upper chamber for aromatic infusion.'],
    [/Masukkan kopi saat air naik, lalu aduk ringan agar aromatik tidak rusak/i, 'Add coffee as the water rises, then stir lightly to protect aromatics.'],
    [/Jaga kontak halus; tambahan aromatik hanya boleh terukur dan aman pangan/i, 'Keep contact gentle; aromatic additions must be measured and food-safe.'],
    [/Matikan panas dan biarkan vakum menyaring seduhan turun/i, 'Turn off the heat and let vacuum suction filter the brew down.'],
    [/Sajikan segera; jangan menambah panas setelah fase turun/i, 'Serve promptly; do not add heat after drawdown.'],
    [/Bilas filter, panaskan Clever, dan siapkan dua fase: rendam lalu alirkan/i, 'Rinse the filter, preheat the Clever, and set up two stages: steep first, then release.'],
    [/Isi fase rendam pertama, basahi semua bubuk, lalu tahan kontak/i, 'Fill the first steep stage, wet all grounds evenly, then hold contact.'],
  ...
  text = text
    ...
    .replace(/\bIsi konsentrat bersih\b/gi, 'Add brew water for a clean concentrate')
    .replace(/\bIsi konsentrat floral\b/gi, 'Add brew water for a floral concentrate')
    .replace(/\bIsi konsentrat padat\b/gi, 'Add brew water for a dense concentrate')
    .replace(/\bIsi konsentrat aromatik\b/gi, 'Add brew water for an aromatic concentrate')
    .replace(/\bIsi konsentrat manis\b/gi, 'Add brew water for a sweet concentrate')
    .replace(/\bbypass terpisah, tidak melewati kopi\b/gi, 'bypass water stays separate and does not pass through the coffee')
    .replace(/\bbypass tidak lewat kopi\b/gi, 'bypass water does not pass through the coffee')
    .replace(/\bbypass di cangkir setelah tekan\b/gi, 'bypass water goes into the cup after pressing')
    ...
    .replace(/\bsuhu\b/gi, 'temp')
    .replace(/\brasa\b/gi, 'taste')
    .replace(/\bpanduan\b/gi, 'guide')
    .replace(/\bkeyakinan\b/gi, 'confidence')
    .replace(/\bacuan\b/gi, 'baseline')
    .replace(/\bvalidasi\b/gi, 'validation')
    .replace(/\bwaktu\b/gi, 'time')
    .replace(/\bekstraksi\b/gi, 'extraction')
    .replace(/\bsetelan\b/gi, 'setting')
    .replace(/\bkalibrasi\b/gi, 'calibration')
    .replace(/\bmantap\b/gi, 'steady')
    .replace(/\bagar\b/gi, 'so')
    .replace(/\bbasah\b/gi, 'wet')
    .replace(/\badukan\b/gi, 'stirring')
    .replace(/\bpekat\b/gi, 'dense')
    .replace(/\bperlahan\b/gi, 'slowly')
    .replace(/\bseluruh\b/gi, 'all')
    .replace(/\blangsung\b/gi, 'directly')
    .replace(/\balas\b/gi, 'bed')
    .replace(/\bpulsa\b/gi, 'pulse')
    .replace(/\bdatar\b/gi, 'flat')
    .replace(/\btanpa\b/gi, 'without')
    .replace(/\bmengguncang\b/gi, 'shaking')
    .replace(/\bsetelah\b/gi, 'after')
    .replace(/\bselesai\b/gi, 'finished')
    .replace(/\bkonsisten\b/gi, 'consistent')
    ...
    .replace(/\bbekerja\b/gi, 'work')
    .replace(/\bpaling\b/gi, 'most')
    .replace(/\bbergelombang\b/gi, 'wavy')
    .replace(/air turun/gi, 'drawdown')
...
function buildAiBrewNextStepCue(step: AiBrewDisplayStep, remainingSeconds: number, language: string) {
  ...
  return isIndonesianAiBrewLanguage(language)
    ? `Berikutnya dalam ${formatGuideTime(remainingSeconds)}`
    : `Next in ${formatGuideTime(remainingSeconds)}`;
}

function buildAiBrewStepQuickNote(step: AiBrewDisplayStep, language: string, plan?: BrewPlan) {
  if (isWorkflowGuideStep(step)) return buildWorkflowGuideActionText(step, language, plan);
  const note = !isIndonesianAiBrewLanguage(language)
    ? translateWorkflowGuideTextToEnglish(step.note)
    : step.note;
  return normalizeAiBrewInstructionText(localizeAiBrewDynamicText(note, language));
}
...
function buildAiBrewStepDetailPoints(
  ...
  const detailText = normalizeAiBrewInstructionText(
    !isIndonesianAiBrewLanguage(language)
      ? localizeAiBrewDynamicText(translateWorkflowGuideTextToEnglish(step.hybridInstruction || ''), language)
...
function formatFlowNextActionValue(step: AiBrewDisplayStep, language: string) {
  const id = isIndonesianAiBrewLanguage(language);
  if (step.pourVolumeMl > 0) return formatRoundedMl(step.pourVolumeMl);
  if (step.targetVolumeMl > 0) return `${id ? 'Target' : 'Target'} ${formatRoundedMl(step.targetVolumeMl)}`;
  const localizedLabel = localizeAiBrewStepLabel(step.label, language);
  const actionType = isWorkflowGuideStep(step) ? step.actionType : String(getAiBrewStepKind(step));
  switch (actionType) {
    case 'press':
      return id ? 'Tekan' : 'Press';
    case 'release':
      return id ? 'Buka katup' : 'Open valve';
    case 'serve':
      return id ? 'Sajikan' : 'Serve';
    case 'stir':
    case 'mix':
    ...
    case 'wait':
    case 'steep':
      return localizedLabel || (id ? 'Tunggu' : 'Wait');
    case 'extract':
      return id ? 'Ekstraksi' : 'Extract';
    default:
      return localizedLabel;
  }
}

function buildCurrentStepAgitationMetric(step: AiBrewDisplayStep | null, language: string): { label: string; value: string } {
  const id = isIndonesianAiBrewLanguage(language);
  const label = id ? 'Agitasi' : 'Agitation';
  if (!step) return { label, value: id ? 'selesai' : 'done' };

  if (isWorkflowGuideStep(step) && step.techniqueChips.length > 0) {
    const actionChip = step.techniqueChips.find((chip) => {
      const text = `${chip.label} ${chip.value}`.toLowerCase();
      return /agitasi|agitation|stir|aduk|swirl|goyang|mix|press|tekan|pulse|pulsa|break|pecah/.test(text);
    });
    if (actionChip) {
      const chipLabel = localizeWorkflowChipLabel(actionChip, language);
      const chipValue = localizeWorkflowChipValue(actionChip.value, language);
      const labelIsAgitation = /^(agitasi|agitation)$/i.test(chipLabel);
      return { label, value: labelIsAgitation ? chipValue : `${chipLabel} ${chipValue}`.trim() };
    }
  }

  const agitation = formatAiBrewAgitation(step.agitationLevel, language);
  if (agitation) return { label, value: agitation };

  const actionType = isWorkflowGuideStep(step) ? step.actionType : String(getAiBrewStepKind(step));
  switch (actionType) {
    case 'stir':
    case 'mix':
      return { label, value: id ? 'aduk terkontrol' : 'controlled stir' };
    case 'press':
      return { label, value: id ? 'tekan stabil' : 'steady press' };
    case 'release':
    case 'serve':
    case 'wait':
    case 'steep':
      return { label, value: 'minimal' };
    default:
      return { label, value: id ? 'terkontrol' : 'controlled' };
  }
}
...
function PlanResultDialog({
  ...
  const methodStyleMetric = buildMethodStyleMetricItem(plan, copy, language);
  const summaryHighlightItemsWithCompletion = insertMetricAfter([
    ...summaryHighlightItems,
    completionTimeItem,
  ], methodStyleMetric, 'temp');
  const detailHighlightItemsWithCompletion = insertMetricAfter(summaryHighlightItemsWithCompletion, methodStyleMetric, 'temp');
  ...
  const localizedExpectedCupConfidence = expectedCup
    ...
    : '';
  const predictionPrecision = buildPredictionPrecision({
    readinessScores: plan.readinessScores || {
      recipe: 50,
      water: 50,
      grinder: 50,
      workflow: plan.workflowValidation?.readinessScore || 50,
      catalog: 50,
    },
    beanCoverage: plan.beanCoverage
      ? {
        category: plan.beanCoverage.category,
        confidence: plan.beanCoverage.confidence,
      }
      : {
  ...
  const predictionPrecisionLabel = predictionPrecision.band === 'high'
    ? (id ? 'Presisi tinggi' : 'High precision')
    : predictionPrecision.band === 'strong'
      ? (id ? 'Presisi kuat' : 'Strong precision')
      : predictionPrecision.band === 'baseline'
        ? (id ? 'Titik awal' : 'Baseline')
        : (id ? 'Perlu diperbaiki' : 'Needs correction');
  ...
  const flowNextActionValue = flowNextStep
    ? formatFlowNextActionValue(flowNextStep, language)
    : copy.flowFinished;
  ...
  const flowCurrentMetrics = flowCurrentStep
    ...
    : { core: [], detail: [] };
  const flowCurrentAgitationMetric = buildCurrentStepAgitationMetric(flowCurrentStep, language);
  ...
  const liteStepAction = flowCurrentStep
    ...
    : (id
      ? 'Ekstraksi utama selesai. Lakukan tahap akhir sesuai panduan; tahap ini tidak termasuk durasi ekstraksi.'
      : 'Main extraction is complete. Follow the finishing step; it is not included in the extraction duration.');
  ...
  const aeropressBypassWaterMl = Math.max(0, Math.round(plan.totalWaterMl - plan.hotWaterMl));
  const aeropressBypassGuideCue = plan.methodFamily === 'aeropress'
    && plan.recipeStyle === 'bypass'
    && aeropressBypassWaterMl > 0
    ? (id
      ? `Tahap bypass: setelah menekan, tambahkan ${formatRoundedMl(aeropressBypassWaterMl)} air bersih ke konsentrat, lalu aduk perlahan sebelum disajikan. Jangan masukkan air bypass ini ke tabung AeroPress.`
      : `Bypass step: after pressing, add ${formatRoundedMl(aeropressBypassWaterMl)} clean water to the concentrate, then stir gently before serving. Do not put this bypass water in the AeroPress chamber.`)
    : '';
    ...
    >
            ...
            <div className="mt-2 w-full max-w-[11.75rem]" data-testid="ai-brew-flow-remaining-status">
              <div className="grid gap-1 text-[10px] leading-4" data-testid="ai-brew-lite-water-status">
                <span className="min-w-0 rounded-xl border border-blue-500/14 bg-[var(--bg-base)] px-2 py-0.5 font-semibold text-secondary [overflow-wrap:anywhere]">
                  {copy.flowNextPour}: <span className="text-primary tabular-nums">{flowNextActionValue}</span>
                </span>
                <span className="min-w-0 rounded-xl border border-blue-500/14 bg-[var(--bg-base)] px-2 py-0.5 font-semibold text-secondary [overflow-wrap:anywhere]">
      ...
      <div className="mx-auto mt-5 max-w-xl space-y-3 rounded-[1.6rem] border border-glass bg-[var(--bg-elevated)] p-4 text-center shadow-[0_14px_32px_rgba(15,23,42,0.08)]" data-testid="ai-brew-flow-current-card">
        ...
        <div className="grid grid-cols-3 gap-1.5 text-[11px]">
          <span className="min-w-0 rounded-xl border panel-divider-subtle bg-[var(--bg-base)]/74 px-2 py-2 text-center text-secondary" data-testid="ai-brew-flow-agitation-metric">
            <span className="block text-[10px] uppercase tracking-widest text-tertiary">{flowCurrentAgitationMetric.label}</span>
            <span className="block truncate font-semibold text-primary">{flowCurrentAgitationMetric.value}</span>
          </span>
        ...
        {aeropressBypassGuideCue && (
          <p
            className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.07] px-4 py-3 text-sm leading-6 text-primary"
            data-testid="ai-brew-aeropress-bypass-guide"
          >
            {aeropressBypassGuideCue}
          </p>
        )}
  ...
  return (
    ...
    >
      <div className="flex h-full min-w-0 max-w-full flex-col overflow-hidden" data-testid="ai-brew-result">
              ...
              <div className="min-w-0">
                <div className="min-w-0 pr-12">
                  <div className="mb-2 flex flex-wrap items-center gap-1.5">
                    ...
                    <span
                      className={`${resultChipClass} inline-flex items-center gap-1.5 ${
                        plan.brewMode === 'iced'
                          ? 'border-sky-500/20 bg-sky-500/10 text-sky-700 dark:text-sky-300'
                          : 'text-primary'
                      }`}
                      data-testid="ai-brew-brew-mode-status"
                    >
                      {plan.brewMode === 'iced' ? <Snowflake size={12} /> : <Droplets size={12} />}
                      {plan.brewMode === 'iced'
                        ? (id ? 'Seduh es' : 'Iced brew')
                        : (id ? 'Seduh panas' : 'Hot brew')}
                    </span>
                  </div>
                ...
                {(expectedCup || readinessItems.length > 0 || plan.beanCoverage) && (
                  <div
                    className="mx-auto mt-3 min-w-0 max-w-full overflow-hidden rounded-xl border panel-divider-subtle bg-[var(--bg-base)]/82 p-2.5 text-xs [overflow-wrap:anywhere]"
                    data-testid="ai-brew-expected-cup"
                  >
                    <div
                      className="grid min-w-0 gap-2"
                      data-testid="ai-brew-prediction-status-card"
                    >
                      <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                        <span
                          className={`rounded-full border px-2 py-1 font-semibold ${
                            predictionPrecision.band === 'high' || predictionPrecision.band === 'strong'
                              ? 'border-emerald-500/18 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'
                              : predictionPrecision.band === 'blocked'
                                ? 'border-rose-500/18 bg-rose-500/10 text-rose-700 dark:text-rose-300'
                                : 'panel-divider-subtle bg-surface-alpha text-primary'
                          }`}
                          data-testid="ai-brew-prediction-precision"
                        >
                          {predictionPrecisionLabel}: {predictionPrecision.score}/100
                        </span>
                      </div>

                      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-lg bg-surface-alpha px-2.5 py-2">
                        <div className="min-w-0">
                          <p className="truncate text-[11px] font-semibold uppercase tracking-[0.16em] text-tertiary">
                            {copy.expectedCupTitle}
                          </p>
                          <p className="mt-0.5 truncate font-semibold text-primary">
                            {expectedCup
                              ? `${copy.cupAcidity} ${expectedCup.acidity}/5 - ${copy.cupSweetness} ${expectedCup.sweetness}/5 - ${copy.cupBody} ${expectedCup.body}/5`
                              : (id ? 'Data keyakinan siap dicek.' : 'Confidence data is ready to review.')}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    {(readinessItems.length > 0 || expectedCup?.warnings[0] || expectedCup?.reasons[0] || plan.beanCoverage) && (
                      <details className="group mt-2 rounded-lg border panel-divider-subtle bg-[var(--bg-base)] px-2.5 py-1.5">
                        <summary
                          className="flex min-h-7 cursor-pointer list-none items-center justify-between gap-3 text-[11px] font-semibold text-secondary"
                          data-testid="ai-brew-prediction-detail-toggle"
                          aria-controls="ai-brew-prediction-detail-body"
                        >
                          <span>{id ? 'Lihat detail' : 'View detail'}</span>
                          <ArrowRight size={13} className="shrink-0 text-secondary transition-transform group-open:rotate-90" />
                        </summary>
                        <div id="ai-brew-prediction-detail-body" className="mt-2 grid gap-2 sm:grid-cols-2">
                          {expectedCup && expectedCupItems.length > 0 && (
                            <div
                              className="rounded-xl bg-surface-alpha px-3 py-2 text-[12px] leading-5 text-secondary sm:col-span-2"
                              data-testid="ai-brew-expected-cup-detail-scores"
                            >
                              <div className="flex min-w-0 flex-wrap items-center justify-between gap-2">
                                <p className="font-semibold text-primary">{id ? 'Skor rasa inti' : 'Core cup scores'}</p>
                                <span className="rounded-full border panel-divider-subtle bg-[var(--bg-base)] px-2 py-0.5 text-[11px] font-semibold text-primary">
                                  {id ? 'Keyakinan' : 'Confidence'}: {localizedExpectedCupConfidence}
                                </span>
                              </div>
                              <div className="mt-2 grid grid-cols-2 gap-1.5 sm:grid-cols-4">
                                {expectedCupItems.slice(0, 4).map((item) => (
                                  <span key={`cup-detail-${item.label}`} className="rounded-lg border panel-divider-subtle bg-[var(--bg-base)] px-2 py-1">
                              ...
                              <p className="mt-2 text-[11px] leading-5 text-secondary">
                                {id
                                  ? 'Skor ini titik awal dari target, alat, air, grinder, roast, dan data bean yang tersedia. Kunci hasil lewat cek rasa.'
                                  : 'These scores are a baseline from the target, brewer, water, grinder, roast, and available bean data. Lock the result through taste feedback.'}
                              </p>
                            </div>
                          )}
                          {plan.beanCoverage && (
                            <div className="rounded-xl bg-surface-alpha px-3 py-2 text-[12px] leading-5 text-secondary" data-testid="ai-brew-bean-coverage-detail">
                          ...
                          {(expectedCup?.warnings[0] || expectedCup?.reasons[0]) && (
                            <div className="rounded-xl bg-surface-alpha px-3 py-2 text-[12px] leading-5 text-secondary">
                          ...
                          {readinessItems.length > 0 && (
                            <div className="grid gap-1.5 rounded-xl bg-surface-alpha px-3 py-2">
                  ...
                  <div className="mt-4" data-testid="ai-brew-result-summary-metric-strip">
                    <div className="grid grid-cols-[repeat(2,minmax(0,1fr))] gap-3 text-xs sm:grid-cols-[repeat(3,minmax(0,1fr))]" data-testid="ai-brew-time-semantics">
                      {summaryHighlightItemsWithCompletion.map((item) => (
                        <span
                          key={item.id}
                          className="min-w-0 max-w-full rounded-2xl border panel-divider-subtle bg-surface-alpha/45 px-4 py-4 text-secondary transition-colors hover:bg-surface-alpha/75"
                        >
                          ...
                          {item.detail && item.id === 'water' && plan.iceMl > 0 ? (
                            <span className="mt-1.5 block break-words text-[11px] leading-4 text-secondary">
                              {id ? 'Panas' : 'Hot'} {formatRoundedMl(plan.hotWaterMl)} +{' '}
                              <span className="font-semibold text-sky-600 dark:text-sky-300" data-testid="ai-brew-ice-accent">
                                {id ? 'es' : 'ice'} {formatRoundedGrams(plan.iceMl)}
                              </span>
                            </span>
                          ) : item.detail ? (
            ...
            {activeTab === 'details' && !isQuickResult && (
              ...
              <div className="grid grid-cols-[repeat(2,minmax(0,1fr))] gap-2.5 sm:grid-cols-[repeat(2,minmax(0,1fr))] xl:grid-cols-[repeat(3,minmax(0,1fr))]">
                {detailHighlightItemsWithCompletion.map((item) => (
                  <div
                    key={`details-${item.id}`}
                    className={resultMetricCardClass}
                    data-testid={`ai-brew-result-${item.id}-metric`}
                  >
              ...
              <div className="grid gap-4">
                ...
                >
                  <button
                    type="button"
                    onClick={() => setTargetCompareExpanded((current) => !current)}
                    className={`flex min-h-9 w-full items-center justify-between gap-3 text-left ${targetCompareExpanded ? 'mb-3' : ''}`}
                    data-testid="ai-brew-target-compare-toggle"
                    aria-expanded={targetCompareExpanded}
                  >
                    ...
                    <ChevronDown size={16} className={`shrink-0 text-secondary transition-transform ${targetCompareExpanded ? 'rotate-180' : ''}`} />
                  </button>
                  {targetCompareExpanded ? <div className="grid gap-2.5">
                    ...
                    <div className="mt-4 rounded-2xl bg-[var(--bg-elevated)] p-3.5 shadow-[0_12px_28px_rgba(15,23,42,0.08)]" data-testid="ai-brew-flow-current-card">
                      ...
                      {isQuickResult ? (
                        <div className="mt-3 grid gap-2 text-xs sm:grid-cols-3">
                          ...
                          <span className="rounded-xl border panel-divider-subtle bg-surface-alpha px-2.5 py-2 text-secondary" data-testid="ai-brew-flow-agitation-metric">
                            <span className="block text-[10px] uppercase tracking-widest text-tertiary">{flowCurrentAgitationMetric.label}</span>
                            <span className="font-semibold text-primary">{flowCurrentAgitationMetric.value}</span>
                          </span>
                      ...
                      {aeropressBypassGuideCue && (
                        <p
                          className="mt-3 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.07] px-3 py-2 text-sm leading-5 text-primary"
                          data-testid="ai-brew-aeropress-bypass-guide"
                        >
                          {aeropressBypassGuideCue}
                        </p>
        ...
        >
          <div className="grid grid-cols-[repeat(2,minmax(0,1fr))] gap-2" data-testid="ai-brew-result-actions">
            <button
              type="button"
              onClick={onSaveRecipe}
              disabled={saving || workflowBlocked}
...
export function AiBrewPanel() {
  ...
  function hydrateInitialState(nextCatalog: AiBrewCatalog) {
    ...
    if (storedPlan) {
      ...
      const defaultForm = createDefaultAiBrewFormState(nextCatalog);
      const draftExists = hasAiBrewFormDraft();
      const draft = loadAiBrewFormDraft(defaultForm);
      const preferredForm = draftExists
        ? draft
        : applyEquipmentPreferencesToForm(draft, equipmentPreferences, {
          dripperIds: new Set(nextCatalog.drippers.map((item) => item.id)),
          grinderIds: new Set(nextCatalog.grinders.map((item) => item.id)),
        });
      const nextForm = sanitizeAiBrewFormState(preferredForm, nextCatalog);
      commitFormState(nextForm);
      setShowMineralEditor(nextForm.waterMode === 'manual');
    }
  ...
  const pickerOptions = useMemo(() => {
    ...
    if (pickerKind === 'dripper') {
      return buildEquipmentPickerOptions(
        sortEquipmentByPreference(catalog.drippers, equipmentPreferences?.preferredDripperId),
        copy,
        'dripper',
        language,
      );
    ...
    return buildEquipmentPickerOptions(
      sortEquipmentByPreference(catalog.grinders, equipmentPreferences?.preferredGrinderId),
      copy,
      'grinder',
      language,
    );
  }, [builderWaterBrands, catalog, copy, equipmentPreferences, language, pickerKind]);
  ...
  function applyFormChange(
    ...
    commitFormState((prev) => {
      ...
      if (clearPreset && key !== 'manualPresetId') {
        next.manualPresetId = '';
      }
      if (key === 'brewMode') {
        const nextMode = value === 'iced' ? 'iced' : 'hot';
        const activeDripper = catalog?.drippers.find((item) => item.id === prev.dripperId);
        if (nextMode === 'hot' && prev.switchPresetId === 'iced_hybrid') {
          next.switchPresetId = '';
        } else if (nextMode === 'iced' && activeDripper?.methodFamily === 'hario_switch') {
          next.switchPresetId = 'iced_hybrid';
        }
      }
      if (
        catalog
  ...
  function updateForm<K extends keyof AiBrewFormState>(key: K, value: AiBrewFormState[K]) {
    const resolution = resolveManualPresetChange({
      activePresetId: formStateRef.current.manualPresetId,
      key,
      value,
    });
    if (resolution.kind === 'confirm_exit') {
      setPendingPresetChange({ key, value });
      return;
    }
    applyFormChange(key, value, resolution.clearPreset);
  }
  ...
  return (
    <div className="ai-brew-compact-shell space-y-5 pb-28 lg:pb-0" data-testid="ai-brew-panel">
      ...
      <div data-testid="ai-brew-preset-change-confirm">
        <ConfirmActionDialog
          open={pendingPresetChange !== null}
          title={isIndonesianAiBrewLanguage(language) ? 'Keluar dari preset?' : 'Leave this preset?'}
          description={isIndonesianAiBrewLanguage(language)
            ? 'Perubahan ini akan melepas preset aktif dan melanjutkan dengan resep kustom. Input lain tetap dipertahankan.'
            : 'This change will detach the active preset and continue as a custom recipe. Your other inputs will be kept.'}
          confirmLabel={isIndonesianAiBrewLanguage(language) ? 'Lanjutkan perubahan' : 'Continue change'}
          cancelLabel={isIndonesianAiBrewLanguage(language) ? 'Tetap gunakan preset' : 'Keep preset'}
          destructive={false}
          onCancel={() => setPendingPresetChange(null)}
          onConfirm={() => {
            const pending = pendingPresetChange;
            setPendingPresetChange(null);
            if (pending) applyFormChange(pending.key, pending.value, true);
          }}
        />
      </div>
