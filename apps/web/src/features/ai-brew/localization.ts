...
export function localizeAiBrewDynamicText(text: string, language?: string) {
  ...
  const localized = normalized
    ...
    .replace(/^Let drawdown finish over the measured ice; stir the server after the final drips so service stays separate from brewing\.$/i, 'Biarkan drawdown selesai di atas es terukur; aduk server setelah tetesan akhir agar tahap saji tetap terpisah dari seduh.')
    ...
    .replace(/Finish at the hot-water target only; let drawdown complete over ice, then stir the server 5-8 seconds before serving\./i, 'Akhiri tepat di target air panas; biarkan drawdown selesai di atas es, lalu aduk server 5-8 detik sebelum saji.')
    .replace(/After the last pour, use only a small leveling swirl if needed, then let drawdown finish without wall-rinsing\./i, 'Setelah tuangan terakhir, pakai swirl kecil untuk meratakan jika perlu, lalu biarkan drawdown selesai tanpa membilas dinding.')
    .replace(/Preheat the chamber and rinse the paper cap first, then wet the compact bed quickly so contact starts evenly\./i, 'Panaskan chamber dan bilas paper cap dulu, lalu basahi bed yang padat dengan cepat agar kontak mulai merata.')
    ...
    .replace(/Rinse the paper and preheat the brewer first, then close the valve before adding coffee and brew water\./i, 'Bilas kertas dan panaskan brewer dulu, lalu tutup valve sebelum menambahkan kopi dan air seduh.')
